...
##################################
## $Revision: 1.4 $ 
## $Id: vdna.tcl,v 1.4 2009/05/12 21:03:09 johns Exp $
## 
...
proc ::VDNA::setvars args {
  ...
  VDNA::geom $VDNA::Geometry
   switch $VDNA::Geometry {
      ...
      "Circular DNA" {
          puts " DNA Circle "     
          set TiltStrng   " 360.0 / \$V1 * sin (\$Pi/180.0 * 36.0 * \$s ) "
          set RollStrng   " 360.0 / \$V1 * cos (\$Pi/180.0 * 36.0 * \$s ) "
          set TwistStrng  " 36.0 "
          set ShiftStrng  " 0 "
          set SlideStrng  " 0 "
          set RiseStrng   " 3.4 "
      ...
      "Shear Helix(+)" {
          ...
          set RollStrng   " 360.0 / \$V1 * cos (\$Pi/180.0 * 36.0 * \$s ) "
          set TwistStrng  " 36.0 "
          set ShiftStrng  " 0.3 * sin (\$Pi/180.0 * 36.0 * \$s ) "
          set SlideStrng  " 0.3 * cos (\$Pi/180.0 * 36.0 * \$s ) "
          set RiseStrng   " 3.4 "
          set sPosition 0
          set sMax      70.0
          set Lk       0
      ...
      "Thermal" {
          puts "Thermal"     
          set TiltStrng   " (360.0 / \$V1 * sin (\$Pi/180.0 * 36.0 * \$s )) * (1.0 + \$V2*(rand()-.5)) "
          set RollStrng   " (360.0 / \$V1 * cos (\$Pi/180.0 * 36.0 * \$s )) * (1.0 + \$V2*(rand()-.5)) "
          set TwistStrng  " 36.0 * (1.0 + \$V2*(rand() -.5))"
          set ShiftStrng  " \$V2*(rand() -.5) "
          set SlideStrng  " \$V2*(rand() -.5) "
          set RiseStrng   " 3.4 * (1.0 +  \$V2*(rand()-.5))"
          set sPosition 0
          set sMax      69.0
          set Lk       0
          set Nuc      0
      ...
      "Chromatin2" {
          ...
          set TiltStrng "(\$s < int (\$s / (\$Lk + \$Nuc ) * (\$Lk + \$Nuc )  + \$Nuc)) ?  \$V1 * sin (\$Pi/180.0 * \$Tw * (\$s -  int (\$s / (\$Lk + \$Nuc ) * (\$Lk + \$Nuc ))) ) : 0 " 
          set RollStrng "(\$s < int (\$s / (\$Lk + \$Nuc ) * (\$Lk + \$Nuc )  + \$Nuc)) ?  \$V1 * cos (\$Pi/180.0 * \$Tw * (\$s -  int (\$s / (\$Lk + \$Nuc ) * (\$Lk + \$Nuc ))) ) : 0 " 
          set TwistStrng  " (\$s < int (\$s / (\$Nuc + \$Lk) ) *  ( \$Nuc + \$Lk )  + \$Nuc ) ?  34.5 : 35.4"
        ...
        "Default" { 
          puts " Default Rod "
          set TiltStrng   " (\$s < \$Nuc ) ? \$V1 * sin ( \$Pi/180.0 * 36.0 * \$s  ) : 0"
          set RollStrng   " (\$s < \$Nuc ) ? \$V1 * cos ( \$Pi/180.0 * 36.0 * \$s  ) : 0"
          set TwistStrng  " 36.0 "
          set ShiftStrng  " (\$s < \$Nuc ) ? \$V2 * sin ( \$Pi/180.0 * 36.0 * \$s) : 0"
          set SlideStrng  " (\$s < \$Nuc ) ? $\V2 * cos ( \$Pi/180.0 * 36.0 * \$s) : 0"
          set RiseStrng   " 3.4 "
          set sPosition 0
          set Lk       30
          set Nuc      146
          set sMax     " 176"
          set IsNucStrng " (\$s < \$Nuc ) ? 1 : 0"
       }
      }
}
...
proc ::VDNA::hp2xyz { } {
  ...
  set frmt "CA     %20.5f %20.5f %20.5f"
  set fp [open "vdna.xyz" "w" ]
  puts $fp [format "     %6d "  [expr int ($sMax) ] ]
  ...
  puts  $fp [format $frmt [lindex $R 0 ] [lindex $R 1 ] [lindex $R 2] ]
...
proc ::VDNA::vdnatk {} {
  ...
  entry $w.vars.sentry -width 10 -relief sunken -bd 2 \
    -textvariable ::VDNA::sMax

  label $w.vars.lklabel -text "Lk: "
  entry $w.vars.lkentry -width 10 -relief sunken -bd 2 \
    -textvariable ::VDNA::Lk

  label $w.vars.nlabel -text "Nuc: "
  entry $w.vars.nentry -width 10 -relief sunken -bd 2 \
    -textvariable ::VDNA::Nuc
  pack $w.vars.slabel $w.vars.sentry $w.vars.lklabel $w.vars.lkentry \

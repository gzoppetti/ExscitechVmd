...
#   
   ...
   variable labelradius
   variable picklist
   variable sphere_coord
   set selatoms $picklist; #[.molefac.val.list.list curselection]
   foreach atom $selatoms {
   set sel [atomselect $tmpmolid "index $atom"]
   lappend atomarktags [graphics $fepmolid color green]
#  lappend atomarktags [graphics $tmpmolid materials on]
#  lappend atomarktags [graphics $tmpmolid material Transparent]
   lappend atomarktags [graphics $fepmolid sphere [join [$sel get {x y z}]] radius [expr $labelradius*1.5]]
   set atomarkcoord [$sel get {x y z}]
   lappend sphere_coord $atomarktags
   lappend sphere_coord $atomarkcoord
   $sel delete
   }
...
#   
   ...
   foreach atom $selatoms {
   set sel [atomselect $tmpmolid "index $atom"]
   lappend atomarktags [graphics $fepmolid color red]
#   lappend atomarktags [graphics $tmpmolid materials on]
#   lappend atomarktags [graphics $tmpmolid material Transparent]
   lappend atomarktags [graphics $fepmolid sphere [join [$sel get {x y z}]] radius [expr $labelradius*1.5]]
   set atomarkcoord [$sel get {x y z}]
...
proc ::Molefacture::flag_commonatoms {} {
   ...
   foreach atom $selatoms {
   set sel [atomselect $tmpmolid "index $atom"]
   set deletelist [lindex $sphere_coord [expr [lsearch $sphere_coord [$sel get {x y z}]] - 1]]
   while { [lsearch $sphere_coord [$sel get {x y z}]] >= 0 } {
   #   puts "deletelist: $deletelist"
   foreach sphere2delete $deletelist {
   graphics $fepmolid delete $sphere2delete
       }
   set sphere_coord [lreplace $sphere_coord [expr [lsearch $sphere_coord [$sel get {x y z}]] - 1] [lsearch $sphere_coord [$sel get {x y z}]]]
   set deletelist [lindex $sphere_coord [expr [lsearch $sphere_coord [$sel get {x y z}]] - 1]]
     }
   $sel delete
   }

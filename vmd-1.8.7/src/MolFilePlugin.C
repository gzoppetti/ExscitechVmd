 ...
 *      $Revision: 1.155 $      $Date: 2009/07/28 21:58:50 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int MolFilePlugin::read_structure(Molecule *m, int filebonds, int autobonds) {
  ...
  molfile_atom_t *atoms = 
    (molfile_atom_t *)malloc(numatoms*sizeof(molfile_atom_t));
  ...
  if (rc) {
    ...
    if (can_read_qm_timestep()) {
      ...
      for (i=0; i<qmmeta.num_wavef; i++) {
        ...
        if (ts->qm_timestep->add_wavefunction(qm_data,
                           qmmeta.wavef_size,
          ...
          int idtag = qm_data->assign_wavef_id(
                                qm_timestep.wave[i].type,
                                qm_timestep.wave[i].spin,
                                qm_timestep.wave[i].excitation,
                                qm_timestep.wave[i].info,
          ...
          ts->qm_timestep->set_wavef_idtag(i, idtag);

          // Update the list of available orbitals
          qm_data->update_avail_orbs(i, 
                            qmmeta.num_orbitals_per_wavef[i],
                            ts->qm_timestep->get_orbitalids(i),
                            ts->qm_timestep->get_occupancies(i));     
        }
      }
      free(signa_ts);
      ...
      if (m->numframes()>1) {
        ...
        prevts = m->get_frame(m->numframes()-2);

        // Sort the orbitals by comparing them to the ones from the previous timestep
        ts->qm_timestep->sort_orbitals(prevts);
      }
...
int MolFilePlugin::write_structure(Molecule *m, const int *on) {
  ...
  // by default anymore.
  if (can_write_bonds() && m->test_dataset_flag(BaseMolecule::BONDS)) {
      ...
      case MOLFILE_TEXT:
        {
          ...
          text[23] = '\0';
          mg->add_text(data, text, graphics[i].size);
        }
        break;
      case MOLFILE_COLOR:
...
int MolFilePlugin::read_qm_data(Molecule *mol) {
  ...
  if (metadata.num_basis_atoms) {
    ...
    qmdata->basis.shell_symmetry   = new int[metadata.num_shells];
...
//     qmdata->run.esp_charges = NULL;
  ...
  if (metadata.have_sysinfo) {
    ...
    mol->qm_data->scftype = qmdata->run.scftype;
    mol->qm_data->runtype = qmdata->run.runtype;
    mol->qm_data->status  = qmdata->run.status;
  ...
  if (!mol->qm_data->init_basis(mol, metadata.num_basis_atoms,
                                ...
                                qmdata->basis.shell_symmetry)) {
    msgErr << "Error reading QM data: Incomplete basis set info."
           << sendmsg;
  }
  ...
  if (metadata.num_basis_atoms) {
    ...
    delete [] qmdata->basis.shell_symmetry;

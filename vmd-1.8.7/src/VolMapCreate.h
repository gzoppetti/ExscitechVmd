 ...
 *      $Revision: 1.89 $       $Date: 2009/05/29 16:20:18 $
 *
 **************************************************************************/
...
public:
  typedef enum
  {
    COMBINE_AVG, COMBINE_MIN, COMBINE_MAX, COMBINE_STDEV, COMBINE_PMF
  } CombineType;
...
protected:
  virtual int
  compute_frame (int frame, float *voldata) = 0;
  int
  compute_init (float padding);

  /// called before computing individual frames
  virtual int
  compute_init ()
  {
    return compute_init (0.);
  }

  /// calculates minmax (into preallocated float[3] arrays) using volmapcreate's selection
  int
  calculate_minmax (float *min_coord, float *max_coord);

  /// calculates max_radius (into a provided float) using volmapcreate's selection
  int
  calculate_max_radius (float &radius);

  /// for checkpointing
  void
  combo_begin (CombineType method, void **customptr, void *params);
  void
  combo_addframe (CombineType method, float *voldata, void *customptr,
      float *framedata);
  void
  combo_export (CombineType method, float *voldata, void *customptr);
  void
  combo_end (CombineType method, void *customptr);
...
public:
  ...
  void
  set_minmax (float minx, float miny, float minz, float maxx, float maxy,
      float maxz);

  void
  set_checkpoint (int checkpointfreq, char *checkpointname);

  int
  compute_all (bool allframes, CombineType method, void *params);
...
public:
  VolMapCreateDensity (VMDApp *app, AtomSel *sel, float res, float *the_weight,
      float the_radscale) :
      VolMapCreate (app, sel, res)
  {
    weight = the_weight;
...
protected:
  ...
  compute_frame (int frame, float *voldata);
public:
  VolMapCreateOccupancy (VMDApp *app, AtomSel *sel, float res,
...
protected:
  ...
  compute_frame (int frame, float *voldata);
public:
  VolMapCreateDistance (VMDApp *app, AtomSel *sel, float res,
      float the_max_dist) :
      VolMapCreate (app, sel, res)
  {
...
public:
  VolMapCreateCoulombPotentialMSM (VMDApp *app, AtomSel *sel, float res) :
      VolMapCreate (app, sel, res)
  {
  }
};
...
private:
  ...
  int
  box_inside_pbccell (int frame, float *minmax);
  ...
  int
  grid_inside_pbccell (int frame, float *voldata, const Matrix4 &alignment);
  ...
  void
  align_frame (Molecule *mol, int frame, float *coords, Matrix4 &alignment);

  // Get array of coordinates of selected atoms and their
  // neighbors (within a cutoff) in the PBC images.
  int
  get_atom_coordinates (int frame, Matrix4 &alignment, int *(&vdwtypes),
      float *(&coords));

  // Check if probe is a linear molecule and returns
  // the Cinf axis.
  int
  is_probe_linear (float *axis);
  ...
  int
  gen_conf_tetrahedral (float *(&conform), int freq, int &numorient,
      int &numrot);

  // Generate conformers for all other symmetries
  int
  gen_conf (float *(&conform), int freq, int &numorient, int &numrot);

  float
  dimple_depth (float phi);
...
public:
  VolMapCreateILS (VMDApp *_app, int molid, int firstframe, int lastframe,
      float T, float res, int subr, float cut, int maskonly);
  ~VolMapCreateILS ();
  ...
  void
  set_probe (int num_probe_atoms, int num_conf, const float *probe_coords,
      const float *vdwrmin, const float *vdweps, const float *charge);
  ...
  void
  set_probe_symmetry (int order1, const float *axis1, int order2,
      const float *axis2, int tetrahedral);
  ...
  void
  set_pbc (float center[3], int bbox);

  // Set maximum energy considered in the calculation.
  void
  set_maxenergy (float maxenergy);

  // Set selection to be used for alignment.
  void
  set_alignsel (AtomSel *asel);

  // Set transformation matrix that was used for the
  // alignment of the first frame.
  void
  set_transform (const Matrix4 *mat);

  int
  get_conformers (float *&conform) const
  {
    conform = conformers;
  ...
  void
  get_statistics (int &numconf, int &numorient, int &numrot)
  {
...
int
volmap_write_dx_file (VolumetricData *volmap, const char *filename);

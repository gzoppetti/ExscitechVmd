...
//   molid   (for addfile only; must be the last item)
    ...
    if (argc == 2) {
      molid = app->molecule_new("molecule", 0);
      if (molid < 0) {
      ...
      for (a=3; a<argc; a+=2) {
        if (!strupncmp(argv[a], "type", CMDLEN)) {
          ...
          if (Tcl_GetBoolean(interp, argv[a+1], &spec.autobonds) != TCL_OK)
            return TCL_ERROR;
        } else if (!strupncmp(argv[a], "filebonds", CMDLEN)) {
          if (Tcl_GetBoolean(interp, argv[a+1], &spec.filebonds) != TCL_OK)
            return TCL_ERROR;
        } else if (!strupncmp(argv[a], "first", CMDLEN)) {
          if (Tcl_GetInt(interp, argv[a+1], &spec.first) != TCL_OK)
            return TCL_ERROR;
        } else if (!strupncmp(argv[a], "last", CMDLEN)) {
          if (Tcl_GetInt(interp, argv[a+1], &spec.last) != TCL_OK)
            return TCL_ERROR;
        } else if (!strupncmp(argv[a], "step", CMDLEN)) {
          if (Tcl_GetInt(interp, argv[a+1], &spec.stride) != TCL_OK)
            return TCL_ERROR;

 ...
 *      $Revision: 1.208 $      $Date: 2009/06/11 14:36:08 $
 *
 ***************************************************************************/
...
struct FileSpec {
  ...
  FileSpec() {
    ...
    waitfor = 1;    // wait for all frames to load
...
public:
  ...
  const char *filerender_name(int n);       ///< Name of nth file render method
  ...
  //Exscitech
  float display_get_screen_width();
  //Exscitech
  ...
  int molecule_new(const char *name, int natoms);
  ...
  /// get/size size of text labels.  This affects all labels.

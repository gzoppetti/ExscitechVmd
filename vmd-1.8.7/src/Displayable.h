 ...
 *	$Revision: 1.85 $	$Date: 2009/04/29 15:42:59 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
public:
  ...
  // Exscitech: Only allow transform for child if parents allow the transform also
  int scaling(void) const
  {
    return doScale && !fixed() && ((parent != NULL) ? parent->scaling() : 1);
  }
  ...
  {
    return doRot && !fixed() && ((parent != NULL) ? parent->rotating() : 1);
  }
  ...
  {
    return doCent && ((parent != NULL) ? parent->cent_translating() : 1);
  }
  ...
  {
    return doGlob && !fixed() && ((parent != NULL) ? parent->glob_translating() : 1);
  }
  // End Exscitech

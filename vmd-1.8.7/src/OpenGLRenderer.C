 ...
 *	$Revision: 1.400 $	$Date: 2009/07/20 16:17:42 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
void
OpenGLRenderer::setup_initial_opengl_state (void)
{
  int i;
  ...
  if (ext->oglrenderer == OpenGLExtensions::ATI)
  {
    ...
    {
      msgInfo
          << "ATI Linux driver detected, limiting features to avoid driver bugs."
          << sendmsg;
      msgInfo << "  Set the environment variable VMDDISABLEATILINUXWORKAROUND"
          << sendmsg;
      msgInfo
          << "  to enable full functionality on a known-safe driver version."
          << sendmsg;
  ...
  {
    msgInfo
        << "WireGL renderer detected, disabling unsupported OpenGL features."
        << sendmsg;
    wiregl = 1;
  ...
  glEnable (GL_NORMALIZE); // automatically normalize normals
      ...
      {
#if defined(_MSC_VER) || defined(MINGW) || defined(__irix) || defined(__APPLE__)
  ...
  if (!(simplegraphics || ogl_acrobat3dcapture))
  {
    ...
    // Exscitech: Commented out and hopefully turned on only when needed
  ...
  if (ext->hastex2d)
  {
    ...
    {
      glTexImage2D (GL_PROXY_TEXTURE_2D, 0, GL_RGB8, sz, sz, 0, GL_RGB,
  ...
  if (ext->hastex3d)
  {
    ...
    {
      GLTEXIMAGE3D (GL_PROXY_TEXTURE_3D, 0, GL_RGB8, sz, sz, sz, 0, GL_RGB,
          GL_UNSIGNED_BYTE, NULL);
  ...
  if (!(simplegraphics || ogl_acrobat3dcapture) && ext->hasglshadinglangarb
      && ext->hasglfragmentshaderarb && ext->hasglvertexshaderarb
  ...
  if (mainshader)
  {
    ...
    {
      shaderpath = (char *) malloc (
          strlen (getenv ("VMDOGLSPHERESHADER")) + 512);
  ...
  else if (mainshader)
  {
    if (glslextensionsavailable)
    {
      msgWarn << "This GPU/driver is buggy, or doesn't fully implement GLSL."
      << sendmsg;
      msgWarn << "Set environment variable VMDGLSLVERBOSE for more info."
      << sendmsg;
    }
  ...
  {
    immersadeskflip = 1;
    msgInfo << "  Enabled Immersadesk right-eye reflection stereo mode"
        << sendmsg;
  }
...
void
OpenGLRenderer::update_lists (void)
{
  // point SphereList to the proper list
  ResizeArray<GLuint> *lists =
      (sphereMode == ::SOLIDSPHERE) ? &solidSphereLists : &pointSphereLists;
...
OpenGLRenderer::OpenGLRenderer (const char *nm) :
    DisplayDevice (nm)
{
  // initialize data
  dataBlock = NULL;
...
void
OpenGLRenderer::set_line_width (int w)
{
  if (w > 0)
  {
...
void
OpenGLRenderer::set_line_style (int s)
{
  if (s == ::DASHEDLINE)
  {
    lineStyle = s;
...
void
OpenGLRenderer::set_sphere_res (int r)
{
...
void
OpenGLRenderer::set_sphere_mode (int m)
{
  // avoid unnecessary display list state changes, helps avoid some serious
  // OpenGL performance problems on MacOS X.
  if (sphereMode == m)
...
void
OpenGLRenderer::cylinder (float *end, float *start, int rod_res,
    float rod_radius, float rod_top_radius)
{
  ...
  R = lenaxis[0] * lenaxis[0] + lenaxis[1] * lenaxis[1]
      + lenaxis[2] * lenaxis[2];
...
static void
cylinder_full (int num, float *edges, int filled)
{
  int n = num;
  float *start = edges;
...
int
OpenGLRenderer::do_define_light (int n, float *color, float *position)
{
  int i;
...
int
OpenGLRenderer::do_activate_light (int n, int turnon)
{
  if (turnon)
  {
...
void
OpenGLRenderer::loadmatrix (const Matrix4 &m)
{
  GLfloat tmpmat[16];
  for (int i = 0; i < 16; i++)
    tmpmat[i] = (GLfloat) (m.mat[i]);
  glLoadMatrixf (tmpmat);
}

void
OpenGLRenderer::multmatrix (const Matrix4 &m)
{
  GLfloat tmpmat[16];
  for (int i = 0; i < 16; i++)
    tmpmat[i] = (GLfloat) (m.mat[i]);
...
void
OpenGLRenderer::abs_screen_loc_3D (float *loc, long *spos)
{
  ...
  if (!gluProject ((GLdouble) (loc[0]), (GLdouble) (loc[1]),
      (GLdouble) (loc[2]), modelMatrix, projMatrix, ogl_viewport, pos, pos + 1,
      pos + 2))
  {
...
void
OpenGLRenderer::abs_screen_loc_2D (float *loc, long *spos)
{
  float newloc[3];
  newloc[0] = loc[0];
  newloc[1] = loc[1];
...
void
OpenGLRenderer::find_3D_from_2D (const float *A3D, const float *B2D, float *B3D)
{
  ...
  if (!gluUnProject ((GLdouble) lsx, (GLdouble) lsy, 0, modelMatrix, projMatrix,
      ogl_viewport, w1, w1 + 1, w1 + 2))
  {
    msgErr << "Can't determine world coords of window position 1." << sendmsg;
    return;
  }
  if (!gluUnProject ((GLdouble) lsx, (GLdouble) lsy, 1.0, modelMatrix,
      projMatrix, ogl_viewport, w2, w2 + 1, w2 + 2))
  {
...
void
OpenGLRenderer::cueing_off (void)
{
  if (cueingAvailable && cueingEnabled)
  {
    glDisable (GL_FOG);
...
void
OpenGLRenderer::culling_on (void)
{
...
void
OpenGLRenderer::culling_off (void)
{
  if (cullingAvailable && cullingEnabled)
  {
    glPolygonMode (GL_FRONT, GL_FILL); // set front face fill mode
...
void
OpenGLRenderer::set_background (const float *newback)
{
  GLfloat r, g, b;
  r = (GLfloat) newback[0];
  g = (GLfloat) newback[1];
...
void
OpenGLRenderer::set_backgradient (const float *topcol, const float *botcol)
{
  int i;
  for (i = 0; i < 3; i++)
  {
...
void
OpenGLRenderer::set_stereo_mode (int newMode)
{
  if (inStereo == newMode)
...
void
OpenGLRenderer::set_cache_mode (int newMode)
{
...
void
OpenGLRenderer::set_render_mode (int newMode)
{
...
void
OpenGLRenderer::normal (void)
{
  // Exscitech: Ensure OpenGL context is set first
  //   If rendering in stereo this would need to be done there also
  makeContextCurrent ();
...
void
OpenGLRenderer::enable_stencil_stereo (void)
{
  int i;
  ...
  if (aaEnabled)
  {
    msgInfo << "Antialiasing must be disabled for scanline interleaved stereo."
        << sendmsg;
    msgInfo << "Antialiasing will be re-enabled when stereo is turned off."
        << sendmsg;
...
void
OpenGLRenderer::disable_stencil_stereo (void)
{
...
// are looking.
void
OpenGLRenderer::set_persp (DisplayEye my_eye)
{
  ...
  if (projection () == PERSPECTIVE)
  {
    ...
    if (shearstereo)
    {
      ...
      glFrustum ((GLdouble) cpLeft + (eyeshift * nearClip / eyeDist),
          ...
          (GLdouble) cpDown, (GLdouble) cpUp, (GLdouble) nearClip,
          (GLdouble) farClip);
      ...
      gluLookAt (eyePos[0], eyePos[1], eyePos[2],
          ...
          (GLdouble) (eyePos[2] + eyeDir[2]), (GLdouble) (upDir[0]),
          (GLdouble) (upDir[1]), (GLdouble) (upDir[2]));
    }
    else
    {
      ...
      glMatrixMode (GL_MODELVIEW);
      glLoadIdentity ();
      gluLookAt (ep[0], ep[1], ep[2], (GLdouble) (eyePos[0] + eyeDir[0]),
          (GLdouble) (eyePos[1] + eyeDir[1]),
          (GLdouble) (eyePos[2] + eyeDir[2]), (GLdouble) (upDir[0]),
          (GLdouble) (upDir[1]), (GLdouble) (upDir[2]));
    }
  ...
  { // ORTHOGRAPHIC
    ...
    fprintf (stderr, "Orthographic call\n");

    glOrtho (-0.25 * vSize * Aspect, 0.25 * vSize * Aspect, -0.25 * vSize,
        0.25 * vSize, nearClip, farClip);
    ...
    glLoadIdentity ();
    gluLookAt (ep[0], ep[1], ep[2], (GLdouble) (eyePos[0] + eyeDir[0]),
        (GLdouble) (eyePos[1] + eyeDir[1]), (GLdouble) (eyePos[2] + eyeDir[2]),
        (GLdouble) (upDir[0]), (GLdouble) (upDir[1]), (GLdouble) (upDir[2]));
  }
...
int
OpenGLRenderer::prepare3D (int do_clear)
{
  if (do_clear)
  {
    clear ();
...
int
OpenGLRenderer::prepareOpaque (void)
{
...
int
OpenGLRenderer::prepareTrans (void)
{
  if (ogl_useblendedtrans)
  {
...
void
OpenGLRenderer::draw_background_gradient (void)
{
...
//**********************************************************************
void
OpenGLRenderer::render (const VMDDisplayList *cmdList)
{
  ...
  for (int cp = 0; cp < VMD_MAX_CLIP_PLANE; cp++)
  {
    ...
    if (cmdList->clipplanes[cp].mode)
    {
      ...
      cpeq[3] = -(cmdList->clipplanes[cp].normal[0]
          * cmdList->clipplanes[cp].center[0]
          + cmdList->clipplanes[cp].normal[1]
              * cmdList->clipplanes[cp].center[1]
          + cmdList->clipplanes[cp].normal[2]
              * cmdList->clipplanes[cp].center[2]);
  ...
  for (int pbcimage = 0; pbcimage < nimages; pbcimage++)
  {
    ...
    if (ogl_cachedebug)
    {
      msgInfo << "Rendering scene: cache enable=" << ogl_cacheenabled
          << ", created=" << ogl_cachecreated << ", serial="
          << (int) cmdList->serial << ", id=" << (int) ogl_cachedid << ", skip="
          << ogl_cacheskip << sendmsg;
    }
    ...
    if ((!ogl_cacheenabled) || ogl_cacheskip
        || (ogl_cacheenabled && ogl_cachecreated))
    {
        ...
        switch (tok)
        {
          ...
          case DPOINTARRAY:
          {
            ...
            if (!(simplegraphics || ogl_acrobat3dcapture))
            {
              ...
              if (ext->hasglpointparametersext
              ...
              // Exscitech: Disable vertex array state
              //   so as not to interfere with other draw calls
          ...
          case DSPHEREARRAY:
          {
            ...
            if (sphereshader && ogl_useglslshader)
            {
              ...
              GLint projloc = GLGETUNIFORMLOCATIONARB (
                  sphereshader->ProgramObject, "vmdprojparms");
...
#define STRIPEWIDTH 32
          ...
          case DVOLUMETEXTURE:
            if (ext->hastex3d)
            {
              ...
              require_volume_texture (cmd->ID, cmd->xsize, cmd->ysize,
                  cmd->zsize, cmd->texmap);
              ...
              if (mainshader && ogl_useglslshader)
              {
                ...
                GLint loc = GLGETUNIFORMLOCATIONARB (mainshader->ProgramObject,
                    "vmdtexturemode");
                GLUNIFORM1IARB (loc, vmdtexturemode);

                // Set active texture map index
                loc = GLGETUNIFORMLOCATIONARB (mainshader->ProgramObject,
                    "vmdtex0");
            ...
            if (!(simplegraphics || ogl_acrobat3dcapture))
            {
              ...
              if (ext->hasmultidrawext)
              {
                ...
                GLMULTIDRAWELEMENTSEXT (GL_TRIANGLE_STRIP,
                    (GLsizei *) vertsperstrip, GL_UNSIGNED_INT,
                    (const GLvoid **) indices, numstrips);
          ...
          case DCONE:
          {
            ...
            cylinder (cmd->pos2, cmd->pos1, cmd->res, cmd->radius,
                cmd->radius2);
          }
            break;
          ...
          case DTEXT:
          {
            ...
            glTranslatef ((wp[0] + textoffset_x) / wp[3],
                (wp[1] + textoffset_y) / wp[3], wp[2] / wp[3]);
          ...
          case DCOLORINDEX:
            ...
            glColor3fv (
                (GLfloat *) (colorData
                    + 3 * (((DispCmdColorIndex *) cmdptr)->color)));
            break;
    ...
    if (ogl_cacheenabled && (!ogl_cacheskip))
    {
      ...
      else
      {
        if (ogl_cachedebug)
        {
          msgInfo << "Calling cached geometry: id=" << (int) ogl_cachedid
              << sendmsg;
        }
...
void
OpenGLRenderer::require_volume_texture (unsigned long ID, unsigned xsize,
    unsigned ysize, unsigned zsize, unsigned char *texmap)
{
  ...
  if ((texName = texturecache.markUsed (ID)) == 0)
  {
    ...
    {
      msgErr << "OpenGLRenderer failed to download 3-D texture map!" << sendmsg;
    }
  }
  else
  { // already cached, so just enable.
    glBindTexture (GL_TEXTURE_3D, texName);
  }
}

int
OpenGLRenderer::build3Dmipmaps (int width, int height, int depth,
    unsigned char *tx)
{
#if defined(GL_VERSION_1_2)
  if (ext->hastex3d)
  {
    ...
    if (xsize > max3DtexX || ysize > max3DtexY || zsize > max3DtexZ)
    {
...
#else
      msgInfo << "Downsampling 3-D texture map from " << width << "x" << height
          << "x" << depth << " to " << xsize << "x" << ysize << "x" << zsize
          << sendmsg;
      ...
      glPixelStorei (GL_UNPACK_ALIGNMENT, 1);
      GLTEXIMAGE3D (GL_TEXTURE_3D, 0, GL_RGB8, xsize, ysize, zsize, 0, GL_RGB,
          GL_UNSIGNED_BYTE, texmap);
...
void
OpenGLRenderer::update_shader_uniforms (void * voidshader, int forceupdate)
{
#if defined(VMDUSEOPENGLSHADER)
  ...
  GLint vmdprojectionmode =
  (ogl_glslprojectionmode == DisplayDevice::PERSPECTIVE) ? 1 : 0;

 ...
 *	$Revision: 1.31 $	$Date: 2009/04/29 15:43:13 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
private:
  ...
  const int *shell_symmetry;      ///< symmetry type (0, 1, 2, ..) per
...
public:
  ...
  Orbital(const float *wave_function,
          const QMData *qmdata,
          const float *atompos, int numatoms,
          int orbid, int atom, int shell); ///< constructor for AO
  ...
  int calculate_mo(DrawMolecule *mol);

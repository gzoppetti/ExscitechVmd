 ...
 *	$Revision: 1.159 $	$Date: 2009/07/29 04:44:21 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
public:
  ...
  void
  generate_volume_isosurface_trimesh (const VolumetricData * v, float isovalue,
      int stepsize);

  void
  generateTriMeshCommand (const VolumetricData * data);
...
private:
  ...
  int
  emitstructwarning (void); ///< warning suppression function
  ...
  static void
  update_lookups (AtomColor *, AtomSel *, ColorLookup *);
  ...
  void
  draw_solid_spheres (float *, int res, float radscale, float fixrad);
  void
  draw_residue_beads (float *, int res, float radscale);
  void
  draw_dotted_spheres (float *, float srad, int sres); ///< dotted reps
  void
  draw_lines (float *, int thickness, float cutoff); ///< lines rep
  void
  draw_cpk_licorice (float *, int, float, int, float, int, int, float cutoff); ///< cpk and licorice reps

#ifdef VMDPOLYHEDRA
  void draw_polyhedra(float *, float); ///< polyhedra reps
#endif
  void
  draw_points (float *, float); ///< points rep
  void
  draw_bonds (float *, float brad, int bres, float cutoff); ///< bonds rep

  /// routine to eliminate cylinder gaps in tube, ribbon and bonds reps
  void
  make_connection (float *prev, float *start, float *end, float *next,
      float radius, int resolution, int is_cyl);

  /// routine to draw a spline curve component, used by tube and ribbon reps
  void
  draw_spline_curve (int num, float *coords, int *idx, int use_cyl, float b_rad,
      int b_res);

  /// ribbon representation
  void
  draw_spline_ribbon (int num, float *coords, float *perps, int *idx,
      int use_cyl, float b_rad, int b_res);

  /// ribbon/tube representations
  void
  draw_spline_new (int num, const float *coords, const float *perps,
      const int *idx, const float *cpwidths, const float *cpheights,
      int numscalefactors, int b_res, int cyclic);

  /// new ribbon representation
  void
  draw_ribbon_from_points (int numpoints, const float *points,
      const float *perps, const int *cols, int numpanels, const float *heights,
      const float *widths, int numscalefactors);

  void
  draw_tube (float *, float radius, int res); ///< tube rep
  void
  draw_ribbons (float *, float brad, int bres, float thickness); ///< ribbon rep
  void
  draw_ribbons_new (float *, float, int, int, float); ///< new ribbon rep
  //@{
  /// ribbon representation helper routines
  int
  draw_protein_ribbons_old (float *, int, float, float, int);
  int
  draw_protein_ribbons_new (float *, int, float, float, int);
  int
  draw_nucleic_ribbons (float *, int, float, float, int, int, int);
  int
  draw_nucleotide_cylinders (float *, int, float, float, int);
  int
  draw_base_sugar_rings (float *, int, float, float, int);
  int
  draw_cartoon_ribbons (float *, int, float, float, int, int);
  //@}

  void
  draw_structure (float *, float brad, int bres, int linethickness); ///< 2ndary structure, cartoon rep
  /// part of cartoon representation
  void
  draw_alpha_helix_cylinders (ResizeArray<float> &x, ResizeArray<float> &y,
      ResizeArray<float> &z, ResizeArray<int> &atom_on, int *color,
      float bond_rad, int bond_res, float *res_start, float *res_end);

  /// part of cartoon representation
  void
  draw_beta_sheet (ResizeArray<float> &x, ResizeArray<float> &y,
      ResizeArray<float> &z, ResizeArray<int> &atom_on, int *color,
      float ribbon_width, float *res_start, float *res_end);

  void
  draw_trace (float *pos, float brad, int bres, int linethickness); ///< C-alpha trace
  void
  draw_dot_surface (float *pos, float srad, int sres, int method); ///< dot surface
  void
  draw_msms (float *pos, int draw_wireframe, int allatoms, float radius,
      float density); ///< MSMS surface from Scripps
  void
  draw_surface (float *pos, int draw_wireframe, float radius); ///< Surf surface from UNC
  void
  draw_hbonds (float *, float maxangle, int thickness, float cutoff); ///< Hydrogen bonds
  void
  draw_dynamic_bonds (float *, float brad, int bres, float cutoff); ///< on-the-fly bond animation

  // Drawing functions for volumetric and isosurface data 
  void
  draw_volslice (int volid, float slice, int axis, int texmode); ///< Volume data, textured slices
  ...
  void
  draw_isosurface (int, float, int, int, int, int); ///< Volume data, isosurface
  void
  draw_volume_field_lines (int volid, float seedval, float minlen, float maxlen,
      float thickness); ///< Volume gradient field lines

#ifdef VMDORBITALS
  void draw_orbital(int, int, int, int, float, int, int, float, int, int); ///< QM orbital isosurface
#endif

  //@{
  // helper functions for volume rendering
  void
  draw_volume_box_solid (const VolumetricData *);
  void
  draw_volume_box_lines (const VolumetricData *);
  void
  draw_volume_slice (const VolumetricData *, int, float, int);
  void
  draw_volume_texture (const VolumetricData *, int);
  void
  draw_volume_isosurface_points (const VolumetricData *, float, int, int);
  void
  draw_volume_isosurface_lit_points (const VolumetricData *, float, int, int);
  void
  draw_volume_isosurface_trimesh (const VolumetricData *, float, int);
  void
  draw_volume_isosurface_lines (const VolumetricData *, float, int, int);
  int
  draw_volume_get_colorid (void);
  void
  prepare_volume_texture (const VolumetricData *v, int method);
  int
  calcseeds_gradient_magnitude (const VolumetricData *v,
      ResizeArray<float> *seeds, float seedmin, float seedmax);
  ...
  void
  create_cmdlist (void); ///< regenerate the command list
  void
  do_create_cmdlist (); ///< used to loop over frames
...
public:
  ...
  enum RegenChoices
  {
    NO_REGEN = 0, MOL_REGEN = 1, SEL_REGEN = 2, REP_REGEN = 4, COL_REGEN = 8
  };
...
private:
  ...
  void
  place_picks (void); ///< create pick points for 'on' atoms

protected:
  virtual void
  do_color_changed (int);
  virtual void
  do_color_rgb_changed (int);
  virtual void
...
public:
  ...
  int
  change_color (AtomColor *); ///< change coloring method
  int
  change_rep (AtomRep *); ///< change representation method
  int
  change_sel (const char *); ///< change atom selection
  void
  force_recalc (int); ///< force a recalculation of everything
  int
  atom_displayed (int); ///< if Nth atom is displayed in ANY rep
  int
  representation_index (void); ///< which representation this is

  int
  set_smoothing (int n)
  { ///< Set window size; 0 for off
    if (n >= 0)
    {
      avgsize = n;
      return TRUE;
  ...
  int
  get_smoothing () const
  { ///< Returns size of smoothing window.
    return avgsize;
  }
  ...
  void
  set_drawframes (const char *frames);
  const char *
  get_drawframes () const
  {
    return framesel;
  }
  ...
  //
  virtual void
  prepare (); ///< prepare for drawing, do updates
  ...
  virtual void
  pick_start (PickMode *p, DisplayDevice *d, int btn, int tag, const int *cell,
  ...
  virtual void
  pick_move (PickMode *p, DisplayDevice *d, int tag, int dim, const float *pos)
  {
    p->pick_molecule_move (mol, d, tag, dim, pos);
  }

  virtual void
  pick_end (PickMode *p, DisplayDevice *d)
  {

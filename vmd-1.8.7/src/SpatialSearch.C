 ...
 *	$Revision: 1.11 $	$Date: 2009/05/04 21:09:21 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
void find_within(const float *xyz, int *flgs, const int *others, 
  ...
  int nthreads;
#ifdef VMDTHREADS
  nthreads = vmd_thread_numprocessors();
  vmd_thread_t * threads = (vmd_thread_t *)calloc(num, sizeof(vmd_thread_t));
#else
  nthreads = 1;
#endif
...
#ifdef VMDTHREADS
  for (i=0; i<nthreads; i++) {
    vmd_thread_create(threads+i, find_within_routine, data+i);
  }
  for (i=0; i<nthreads; i++) {

 ...
 *	$Revision: 1.111 $	$Date: 2009/05/15 19:17:32 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
void *
Displayable::operator new (size_t n)
{
  return vmd_alloc (n);
}

void
Displayable::operator delete (void *p, size_t)
{
  vmd_dealloc (p);
}

Displayable::Displayable (Scene *s) :
  scene (s)
{
...
Displayable::Displayable (Displayable *pops) :
  scene (pops->scene)
{
...
void
Displayable::reset_disp_list (void)
{
  _needUpdate = 1;
...
void
Displayable::add_child (Displayable *d)
{
...
int
Displayable::remove_child (Displayable *d)
{
...
void
Displayable::draw (DisplayDevice *d) const
{
  ...
  if (isOn)
  {
   /* if (glIsEnabled (GL_VERTEX_ARRAY))
      ...
      fprintf (stderr, "not enabled yet\n");*/
   // fprintf (stderr, "drawing something that has %d children\n", num_children);
    d->render (cmdList);
    for (int i = 0; i < num_children; i++)
    {
      //fprintf (stderr, "drawing child %d\n", i);
      child (i)->draw (d);
      //fprintf (stderr, "done drawing child %d\n", i);
    }
  }
}
...
void
Displayable::reset_transformation (void)
{
...
void
Displayable::set_scale (float s)
{
  if (fixed ())
    return; // only transform unfixed objects
...
void
Displayable::mult_scale (float s)
{
  if (fixed ())
    return; // only transform unfixed objects
...
void
Displayable::add_rot (float x, char axis)
{
  if (fixed ())
    return; // only transform unfixed objects
...
void
Displayable::set_rot (float x, char axis)
{
  if (fixed ())
    return; // only transform unfixed objects
...
void
Displayable::add_rot (const Matrix4 &m)
{
  if (fixed ())
    return; // only transform unfixed objects
...
void
Displayable::set_rot (const Matrix4 &m)
{
  if (fixed ())
    return; // only transform unfixed objects
...
void
Displayable::set_glob_trans (float x, float y, float z)
{
  if (fixed ())
    return; // only transform unfixed objects
...
void
Displayable::add_glob_trans (float x, float y, float z)
{
  if (fixed ())
    return; // only transform unfixed objects
...
void
Displayable::set_cent_trans (float x, float y, float z)
{
  // do trans for all children as well
  for (int i = 0; i < num_children; i++)
...
void
Displayable::add_cent_trans (float x, float y, float z)
{
  // do trans for all children as well
  for (int i = 0; i < num_children; i++)
...
void
Displayable::change_center (float x, float y, float z)
{
  // Here's the math:
...
void
Displayable::change_material (const Material *mat)
{
  _needUpdate = 1;
...
int
Displayable::curr_material () const
{
  return cmdList->materialtag;
}

void
Displayable::update_material (const Material *mat)
{
  if (mat->ind == curr_material ())
    change_material (mat);
  for (int i = 0; i < num_children; i++)
    children[i]->update_material (mat);
}

void
Displayable::delete_material (int n, const MaterialList *mlist)
{
  if (n == curr_material ())
  {
  ...
  for (int i = 0; i < num_children; i++)
    children[i]->delete_material (n, mlist);
}
...
VMDDisplayList *
Displayable::pick_cmd_list (void)
{
  return cmdList;
}
...
int
Displayable::pickable_on (void)
{
  return displayed ();
}

void
Displayable::color_changed (int cat)
{
  do_color_changed (cat);
  for (int i = 0; i < num_children; i++)
    child (i)->color_changed (cat);
}

void
Displayable::color_rgb_changed (int color)
{
  do_color_rgb_changed (color);
  for (int i = 0; i < num_children; i++)
    child (i)->color_rgb_changed (color);
}

void
Displayable::color_scale_changed ()
{
  do_color_scale_changed ();
  for (int i = 0; i < num_children; i++)
    child (i)->color_scale_changed ();
}

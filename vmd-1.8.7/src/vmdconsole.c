 ...
 *      $Revision: 1.8 $       $Date: 2009/04/29 15:43:39 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
static vmd_mutex_t vmdcon_status_lock;
static vmd_mutex_t vmdcon_output_lock;

/* initialize vmdcon */
void vmdcon_init(void) 
{
    vmd_mutex_init(&vmdcon_status_lock);
    vmd_mutex_init(&vmdcon_output_lock);
...
int vmdcon_showlog(void)
{
    ...
    vmd_mutex_lock(&vmdcon_output_lock);
    log=vmdcon_logmsgs;
    do {
    ...
    vmd_mutex_unlock(&vmdcon_output_lock);
    return vmdcon_purge();
}

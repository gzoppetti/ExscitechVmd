 ...
 *	$Revision: 1.58 $	$Date: 2009/05/15 19:17:33 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
void MoleculeGraphics::create_cmdlist(void) {
  ...
  for (int i=0; i<num; i++) {
    ...
    case TEXT: {
      ...
      text.putdata(shape->data, shapetext[(int)shape->data[4]],cmdList);
      append(DMATERIALON);
      break;
    }
    case DCOMMENT: {
    ...
    case MATERIAL: {
      ...
      cmdList->opacity = data[4];
      cmdList->outline = data[5];
      cmdList->outlinewidth = data[6];
      cmdList->materialtag = (int)data[7];
      break;
    }
    case COLOR: {
...
int MoleculeGraphics::add_text(const float *x, const char *text, float size) {
  ShapeClass s(TEXT, 5, next_id); 
  float *data = s.data;
  vec_copy(data+0, x);
  data[3] = size;
  data[4] = (float)shapetext.num(); // index where the text will be stored
...
int MoleculeGraphics::use_material(const Material *mat) {
  ShapeClass s(MATERIAL, 8, next_id);
  float *data = s.data;
  data[0] = mat->ambient;
  data[1] = mat->specular;
  data[2] = mat->diffuse;
  ...
  data[4] = mat->opacity;
  data[5] = mat->outline;
  data[6] = mat->outlinewidth;
  data[7] = (float)mat->ind;
...
const char *MoleculeGraphics::info_id(int id) {
  ...
  case TEXT: {
    sprintf(graphics_info, "text {%f %f %f} {%s} size %f",
            shape->data[0], shape->data[1], shape->data[2],
            shapetext[(int)shape->data[4]], shape->data[3]);
    return graphics_info;
  }
  case MATERIALS: {
  ...
  case MATERIAL: {
    sprintf(graphics_info, "material %d", int(shape->data[7]));
    return graphics_info;
  }
  case COLOR: {

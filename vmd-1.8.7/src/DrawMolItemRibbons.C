 ...
 *	$Revision: 1.137 $	$Date: 2009/04/29 15:43:00 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
*/
  ...
  if (pickpointindices.num() > 0) {
    ...
    pickPointIndexArray.putdata(pickpointindices.num(), &pickpointindices[0], cmdList);
  }
}
...
int DrawMolItem::draw_protein_ribbons_new(float *framepos, int b_res, float b_rad,
  ...
  for (frag=0; frag<mol->pfragList.num(); frag++) {
    ...
    if (modulate != NULL) {
      ...
      int i;
      for (i=0; i<num; i++) {
        widths[i] = 7 * ribbon_width * b_rad + modulate[i];
        heights[i] = b_rad;
      }
    ...
    for (loop=0; loop<num; loop++) {
      ...
      if (resnameindex == nuctypes[0] || resnameindex == nuctypes[1]) {
        ...
        if (o4num < 0 || !(atomSel->on[o4num]))
          continue;
        iend = o4num;
      } else if (resnameindex == nuctypes[8] || resnameindex == nuctypes[9]) {
        // URA
        int o4num = mol->find_atom_in_residue("O4", res);
        if (o4num < 0 || !(atomSel->on[o4num]))
          continue;
        ...
        // use random residue names that will never be recognized by VMD.
        int o4num = mol->find_atom_in_residue("O4", res);
        if (o4num < 0 || !(atomSel->on[o4num]))
          continue;
        iend = o4num;
...
int DrawMolItem::draw_cartoon_ribbons(float *framepos, int b_res, float b_rad,
  ...
  vmd_timerhandle tm = vmd_timer_create();
  vmd_timerhandle tm2 = vmd_timer_create();
  vmd_timerhandle tm3 = vmd_timer_create();
  vmd_timerhandle tm4 = vmd_timer_create();
  vmd_timer_start(tm);
  vmd_timer_start(tm3);
  double foo=0.0;
#endif
...
#if defined(VMDFASTRIBBONS)
  vmd_timer_stop(tm3);
  msgInfo << "Cartoon nucleotide time: " << vmd_timer_time(tm3) << sendmsg;
#endif
...
#if defined(VMDFASTRIBBONS)
  vmd_timer_stop(tm2);
  msgInfo << "Cartoon spline time: " << vmd_timer_time(tm2) << sendmsg;
  msgInfo << "     subspline time: " << foo << sendmsg;
#endif
#if defined(VMDFASTRIBBONS)
  vmd_timer_stop(tm);
  msgInfo << "Cartoon regen time: " << vmd_timer_time(tm) << sendmsg;
  vmd_timer_destroy(tm);
  vmd_timer_destroy(tm2);
  vmd_timer_destroy(tm3);
#endif

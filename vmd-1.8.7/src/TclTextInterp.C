 ...
 *      $Revision: 1.105 $      $Date: 2009/04/29 15:43:27 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
#if defined(_MSC_VER) || defined(MINGW) && TCL_MINOR_VERSION >= 5
...
int TclTextInterp::evalFile(const char *fname) {
  ...
  while ((length = Tcl_GetsObj(inchannel, cmdPtr)) >= 0) {
    ...
    // ExSciTech: Good question, let's remove it.
    //app->display_update();

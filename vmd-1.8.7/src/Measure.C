 ...
 *      $Revision: 1.124 $       $Date: 2009/07/13 23:58:01 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int measure_center(const AtomSel *sel, const float *framepos,
  ...
  int i, j;
  float x, y, z, w;
  for (i=0,j=0,w=0,x=0,y=0,z=0; i<sel->num_atoms; i++) {
    if (sel->on[i]) {
      float tw = weight[j];
      w += tw;
      x += tw * framepos[3*i  ];
...
int measure_rgyr(AtomSel *sel, MoleculeList *mlist, const float *weight, 
                 float *rgyr) {
  ...
  int i, j;
  float total_w, w, sum;
  for (i=0,j=0,total_w=0,sum=0; i<sel->num_atoms; i++) {
    if (sel->on[i]) {
      w = weight[j];
      total_w += w;
...
int measure_rmsd(AtomSel *sel1, AtomSel *sel2, int num, const float *framepos1,
                 const float *framepos2, float *weight, float *rmsd) {
  ...
  int sel1ind = 0; // start from the first atom
  int sel2ind = 0;
  float wsum = 0;
  float rmsdsum = 0;
  ...
  *rmsd = 10000000; // if we bail out, return a huge value
  ...
  while (count-- > 0) {
    ...
    // loop is safe since we already check the on count > 0 previously
...
int measure_fit(const AtomSel *sel1, const AtomSel *sel2, const float *x, 
  ...
  if (!opt || strcmp(opt, "oldvmd")) {
    ...
    v1 = new float[3*num];
    v2 = new float[3*num];
    for (i=0, k=0; i<sel1->num_atoms; i++) {
      if (sel1->on[i]) {
        int ind = 3 * i;
        v1[k++] = x[ind    ];
        v1[k++] = x[ind + 1];
    ...
    for (i=0, k=0; i<sel2->num_atoms; i++) {
      if (sel2->on[i]) {
        int ind = 3 * i;
        v2[k++] = y[ind    ];
        v2[k++] = y[ind + 1];
...
extern "C" void * measure_gofr_orth(void *voidparms) {
  ...
  for (i=count_o_start; i<count_o_end; ++i) {
    ...
    if (msgtp && msg_timer_timeout(msgtp)) {
      char tmpbuf[1024];
      if (msgcnt==0) 
...
int measure_gofr(AtomSel *sel1, AtomSel *sel2, MoleculeList *mlist,
  ...
  msgtimer *msgt = msg_timer_create(5);
  ...
  threads = new vmd_thread_t[numprocs];
  memset(threads, 0, numprocs * sizeof(vmd_thread_t));
...
#if defined(VMDTHREADS)
      for (i=0; i<numprocs; ++i) {
        vmd_thread_create(&threads[i], measure_gofr_orth, &parms[i]);
      }
      for (i=0; i<numprocs; ++i) {

 ...
 *	$Revision: 1.105 $	$Date: 2009/07/10 22:43:57 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
//   printf("num_wave_f/wave_size = %d/%d\n", num_wave_f,  wave_size);
  ...
  for (i=0; i<numatoms; i++) {
    const basis_atom_t *basis_atom = &basis_set[atom_types[i]];
    for (j=0; j<basis_atom->numshells; j++) {
      int symmetry = basis_atom->shell[j].symmetry;
#ifdef DEBUGORBS
      printf("atom %i/%i,  %i/%i %c-shell\n", i+1, numatoms, j+1, basis_atom->numshells, shellname[symmetry]);
#endif
      for (k=0; k<basis_atom->shell[j].num_cart_func; k++) {
        wave_f[ifunc] = wfn[ifunc] * norm_factors[symmetry][k];

#ifdef DEBUGORBS
        printf("%3i %c %2i wave_f[%3i]=% f  norm=%.3f  normwave=% f\n",
               i, shellname[symmetry], k, ifunc, wfn[ifunc],
               norm_factors[symmetry][k], wave_f[ifunc]);
#endif
        ifunc++;
      }
    }
...
int Orbital::calculate_mo(DrawMolecule *mol) {
  ...
  grid_data = new float[numgridpoints*sizeof(float)];
...
#if defined(VMDCUDA)
  // XXX hack to ease testing for now
  ...
  if (rc!=0)  rc = evaluate_grid_fast(numatoms, wave_f, basis_array,
                                      ...
                                      shell_symmetry, numvoxels, voxelsize, 
                                      origin, grid_data);
...
#if 0
  { 
    double gflops = (numgridpoints * flops_per_gridpoint()) / (vmd_timer_time(timer) * 1000000000.0);

    char strbuf[1024];
    sprintf(strbuf, "Orbital calc. time %.3f secs, %.2f gridpoints/sec, %.2f GFLOPS",
            vmd_timer_time(timer), 
            (((double) numgridpoints) / vmd_timer_time(timer)),
            gflops);
...
float Orbital::evaluate_grid_point(float grid_x, float grid_y, float grid_z) {
  ...
  for (at=0; at<numatoms; at++) {
    ...
    for (shell=0; shell < maxshell; shell++) {
      ...
      for (j=0, zdp=1.0f; j<=shell_type; j++, zdp*=zdist) {
...
#else
                int i, j, k;
                for (k=0; k<=shell_type; k++) {
                  for (j=0; j<=shell_type; j++) {
                    for (i=0; i<=shell_type; i++) {
                      if (i+j+k==shell_type) {
                        value += wave_f[ifunc++] * contracted_gto
...
// 
#if defined(_MSC_VER) || defined(MINGW) /* visual c++ */
    ...
    for (ny=0; ny<numvoxels[1]; ny++) {
      ...
      for (nx=0; nx<numvoxels[0]; nx+=4) {
        ...
        for (at=0; at<numatoms; at++) {
          ...
          __m128 ydist2 = _mm_mul_ps(xdist, xdist);
          __m128 zdist2 = _mm_mul_ps(xdist, xdist);
          ...
          for (shell=0; shell < maxshell; shell++) {
            ...
            switch (shell_type) {
              ...
              case F_SHELL:
                ...
                tmpshell = _mm_add_ps(tmpshell, _mm_mul_ps(_mm_load_ps1(&wave_f[ifunc++]), _mm_mul_ps(_mm_mul_ps(xdist, zdist), zdist)));
...
extern "C" void * orbitalthread(void *voidparms) {
  ...
  vmd_tasktile_t tile;
  while (vmd_threadlaunch_next_tile(voidparms, 2, &tile) != VMD_SCHED_DONE) {
    int k;
...
int evaluate_grid_fast(int numatoms,
  ...
  rc = vmd_threadlaunch(numprocs, &parms, orbitalthread, &tile);
...
void Orbital::print_wavefunction() {
  ...
  // XXX Windows doesn't provide log2f(), nor log2() ?!?!?!?!
  ...
  for (at=0; at<numatoms; at++) {
    for (shell=0; shell < num_shells_per_atom[at]; shell++) {
      int shell_type = basis_set[at].shell[shell].symmetry;
      ...
      for (j=0, zdp=1.0f; j<=shell_type; j++, zdp*=zdist) {
        int imax = shell_type - j; 
        for (i=0, ydp=1.0f, xdp=pow(xdist, imax); i<=imax; i++, ydp*=ydist, xdp*=xdiv) {
          printf("%3i %c", at, shellname[shell_type]);
          int k, m=0;
          char buf[20]; buf[0] = '\0';

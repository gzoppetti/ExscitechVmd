 ...
 *      $Revision: 1.17 $       $Date: 2009/05/15 19:17:34 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
static PyObject *change(PyObject *self, PyObject *args, PyObject *keywds) {
  char *name;
  float ambient, specular, diffuse, shininess, opacity, outline, outlinewidth;

  static char *kwlist[] = {
    (char *)"name", (char *)"ambient", (char *)"specular", (char *)"diffuse",
    (char *)"shininess", (char *)"opacity", 
    (char *)"outline", (char *)"outlinewidth", NULL
  };

  if (!PyArg_ParseTupleAndKeywords(args, keywds, (char *)"s|fffff", kwlist,
    &name, &ambient, &specular, &diffuse, &shininess, &opacity, 
    &outline, &outlinewidth))
    return NULL;
  ...
  if (PyDict_GetItemString(keywds, (char *)"outline"))
    app->material_change(name, MAT_OPACITY, outline);
  if (PyDict_GetItemString(keywds, (char *)"outlinewidth"))
    app->material_change(name, MAT_OPACITY, outlinewidth);

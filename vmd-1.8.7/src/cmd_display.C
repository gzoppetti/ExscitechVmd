 ...
 *      $Revision: 1.65 $       $Date: 2009/05/18 15:16:30 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int text_cmd_display(ClientData cd, Tcl_Interp *interp, int argc, 
  ...
  if (argc <= 1) {
    Tcl_SetResult(interp,
       ...
       "             depthcue | culling | size | stereo  | stereomodes |\n"
       ...
       "         depthcue | culling | cachemode | rendermode | stereo |\n"
       "         shadows | ambientocclusion | backgroundgradient> newvalue\n"
  ...
  if (argc == 3 && !strupncmp(argv[1], "get", CMDLEN)) {
	...
	"stereo stereomodes nearclip farclip\n" 
        "cuestart cueend cuedensity cuemode\n"
        "shadows, ambientocclusion, aoambient, aodirect\n",
        TCL_STATIC);
	return TCL_ERROR;
  ...
  if(argc == 2) {
    ...
    // XXX undocumented until it works on all platforms

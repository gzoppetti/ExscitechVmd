 ...
 *	$Revision: 1.111 $	$Date: 2009/07/15 23:08:17 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
static int
parse_minmax_args (Tcl_Interp *interp, int arg_minmax, Tcl_Obj * const objv[],
    double *minmax)
{
  int num, i;
  ...
  if (Tcl_ListObjGetElements (interp, objv[arg_minmax], &num, &data) != TCL_OK)
  {
    Tcl_SetResult (interp,
        (char *) "volmap: could not read parameter (-minmax)", TCL_STATIC);
    return TCL_ERROR;
  }
  if (num != 2)
  {
    Tcl_SetResult (interp,
        (char *) "volmap: minmax requires a list with two vectors (-minmax)",
        TCL_STATIC);
    return TCL_ERROR;
  }
  ...
  if (num != 3)
  {
    Tcl_SetResult (
        interp,
        (char *) "volmap: the first vector does not contain 3 elements (-minmax)",
        TCL_STATIC);
    return TCL_ERROR;
  }
  ...
  if (num != 3)
  {
    Tcl_SetResult (
        interp,
        (char *) "volmap: the second vector does not contain 3 elements (-minmax)",
        TCL_STATIC);
    return TCL_ERROR;
  }
  ...
  if (minmax[0] >= minmax[3] || minmax[1] >= minmax[4]
      || minmax[2] >= minmax[5])
  {
    Tcl_SetResult (interp, (char *) "volmap: invalid minmax range (-minmax)",
        TCL_STATIC);
    return TCL_ERROR;
  }
...
static int
vmd_volmap_ils (VMDApp *app, int argc, Tcl_Obj * const objv[],
    Tcl_Interp *interp)
{
  ...
  if (!app->molecule_valid_id (molid))
  {
    Tcl_AppendResult (interp,
        "volmap: molecule specified for ouput is invalid. (-mol)", NULL);
    return TCL_ERROR;
  }
  ...
  for (arg = 3; arg < argc && !bad_usage && !bailout; arg++)
  {
    ...
    if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-res"))
    {
      ...
      {
        Tcl_AppendResult (interp,
            "volmap ils: resolution must be positive. (-res)", NULL);
        bailout = true;
        break;
      }
      arg++;
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-probesel"))
    {
      ...
      probesel = tcl_commands_get_sel (interp,
          Tcl_GetStringFromObj (objv[arg + 1], NULL));
      if (!probesel)
      {
        Tcl_AppendResult (interp, "volmap ils -probesel: no atom selection.",
            NULL);
        bailout = true;
        break;
      }
      ...
      {
        Tcl_AppendResult (interp, "volmap ils -probesel: no atoms selected.",
            NULL);
        bailout = true;
        break;
      }
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-first"))
    {
      ...
      {
        Tcl_AppendResult (interp,
            "volmap ils: Frame specified with -first must be positive.", NULL);
        bailout = true;
        break;
      }
      if (first >= nframes)
      {
        Tcl_AppendResult (
            interp,
            "volmap ils: Frame specified with -first exceeds number of existing frames.",
            NULL);
        bailout = true;
        break;
      }
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-last"))
    {
      ...
      {
        Tcl_AppendResult (interp,
            "volmap ils: Frame specified with -last must be positive.", NULL);
        bailout = true;
        break;
      }
      if (last >= nframes)
      {
        Tcl_AppendResult (
            interp,
            "volmap ils: Frame specified with -last exceeds number of existing frames.",
            NULL);
        bailout = true;
        break;
      }
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-cutoff"))
    {
      ...
      {
        Tcl_AppendResult (interp,
            "volmap ils: cutoff must be positive. (-cutoff)", NULL);
        bailout = true;
        break;
      }
      arg++;
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-probecoor"))
    {
      ...
      double tmp;
      Tcl_Obj **listObj;
      if (Tcl_ListObjGetElements (interp, objv[arg + 1], &nprobecoor,
          &listObj) != TCL_OK)
      {
        Tcl_AppendResult (interp, " volmap ils: bad syntax in probecoor!",
            NULL);
        bailout = true;
        break;
      }
      ...
      for (i = 0; i < nprobecoor; i++)
      {
        Tcl_Obj **coorObj;
        int j, ndim = 0;
        if (Tcl_ListObjGetElements (interp, listObj[i], &ndim,
            &coorObj) != TCL_OK)
        {
          Tcl_AppendResult (interp, " volmap ils: bad syntax in probecoor!",
              NULL);
        ...
        {
          Tcl_AppendResult (interp,
              " volmap ils: need three values for each probecoor vector", NULL);
          bailout = true;
          break;
        }
        ...
        for (j = 0; j < 3; j++)
        {
          ...
          {
            Tcl_AppendResult (interp, " volmap ils: non-numeric in probecoor",
                NULL);
            bailout = true;
            break;
          }
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-probevdw"))
    {
      ...
      double tmp;
      Tcl_Obj **listObj;
      if (Tcl_ListObjGetElements (interp, objv[arg + 1], &nprobevdw,
          &listObj) != TCL_OK)
      {
      ...
      for (i = 0; i < nprobevdw; i++)
      {
        ...
        if (Tcl_ListObjGetElements (interp, listObj[i], &ndim,
            &vdwObj) != TCL_OK)
        {
          Tcl_AppendResult (interp, " volmap ils: bad syntax in probevdw",
              NULL);
          bailout = true;
          break;
        }
        ...
        {
          Tcl_AppendResult (interp,
              " volmap ils: Need two probevdw values (eps, rmin) for each atom",
              NULL);
          bailout = true;
          break;
        }
        ...
        {
          Tcl_AppendResult (interp,
              " volmap ils: Non-numeric in probevdw (eps)", NULL);
          bailout = true;
          break;
        }
        ...
        {
          Tcl_AppendResult (interp,
              " volmap ils: Non-numeric in probevdw (rmin)", NULL);
          bailout = true;
          break;
        }
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-probecharge"))
    {
      ...
      double tmp;
      Tcl_Obj **listObj;
      if (Tcl_ListObjGetElements (interp, objv[arg + 1], &nprobecharge,
          &listObj) != TCL_OK)
      {
        Tcl_AppendResult (interp, " volmap ils: bad syntax in probecharge",
            NULL);
        bailout = true;
        break;
      }
      ...
      for (i = 0; i < nprobecharge; i++)
      {
        ...
        {
          Tcl_AppendResult (interp, " volmap ils: non-numeric in probecharge",
              NULL);
          bailout = true;
          break;
        }
        ...
        || !strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-conf"))
    {
      ...
      if (num_conf < 0)
      {
        Tcl_AppendResult (interp, "volmap ils: invalid -orient parameter",
            NULL);
        bailout = true;
        break;
      }
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-subres"))
    {
      ...
      if (subres < 1)
      {
        Tcl_AppendResult (interp, "volmap ils: invalid -subres parameter",
            NULL);
        bailout = true;
        break;
      }
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-maxenergy"))
    {
      ...
      {
        Tcl_AppendResult (interp, "volmap ils: invalid -maxenergy parameter",
            NULL);
        bailout = true;
        break;
      }
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-alignsel"))
    {
      ...
      alignsel = tcl_commands_get_sel (interp,
          Tcl_GetStringFromObj (objv[arg + 1], NULL));
      if (!alignsel)
      {
        Tcl_AppendResult (interp, "volmap: no atom selection for alignment.",
            NULL);
        bailout = true;
        break;
      }
      ...
      {
        Tcl_AppendResult (interp, "volmap: no atoms selected for alignment.",
            NULL);
        bailout = true;
        break;
      }
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-pbccenter"))
    {
      ...
      double tmp;
      Tcl_Obj **centerObj;
      if (Tcl_ListObjGetElements (interp, objv[arg + 1], &ndim,
          &centerObj) != TCL_OK)
      {
      ...
      {
        Tcl_AppendResult (interp,
            " volmap ils: need three values for vector pbccenter", NULL);
        bailout = true;
        break;
      }
      ...
      for (i = 0; i < 3; i++)
      {
        ...
        {
          Tcl_AppendResult (interp, " volmap ils: non-numeric in pbccenter",
              NULL);
          bailout = true;
          break;
        }
  ...
  if (!bailout)
  {
    if (probesel)
    {
      ...
      {
        Tcl_AppendResult (interp,
            "volmap ils: Must specify either -probesel or -probecoor not both.",
            NULL);
        bailout = true;
      }
      ...
      {
        Tcl_AppendResult (interp,
            "volmap ils: probe selection contains no VDW radii", NULL);
        bailout = true;
      }
      ...
      if (!occupancy)
      {
        Tcl_AppendResult (
            interp,
            "volmap ils: probe selection contains no VDW epsilon values (in occupancy field)",
            NULL);
        bailout = true;
      }
    ...
    else
    {
      ...
      {
        Tcl_AppendResult (interp,
            "volmap ils: No probe VDW parameters specified.", NULL);
        bailout = true;
      }
      ...
      {
        Tcl_AppendResult (interp,
            "volmap ils: # probe charges doesn't match # probe atoms", NULL);
        bailout = true;
      }
    }
    ...
    {
      Tcl_AppendResult (interp, "volmap: No probe coordinates specified.",
          NULL);
      bailout = true;
    }
    ...
    {
      Tcl_AppendResult (interp,
          "volmap ils: # probe VDW params doesn't match # probe atoms", NULL);
      bailout = true;
    }
  ...
  if (probesel)
  {
    ...
    sym.set_overlaptol (0.05);

    // Take bonds into account
    sym.set_checkbonds (1);

    // Guess the symmetry
    int ret_val = sym.guess (0.05);
    if (ret_val < 0)
    {
      Tcl_AppendResult (interp, "measure symmetry: ", measure_error (ret_val),
          NULL);
      return TCL_ERROR;
    }
    int pgorder;
  ...
  VolMapCreateILS vol (app, molid, first, last, (float) temperature,
      (float) resolution, subres, (float) cutoff, maskonly);

  vol.set_probe (num_probe_atoms, num_conf, probe_coords, probe_vdwrmin,
      probe_vdweps, probe_charge);
  vol.set_maxenergy (float (maxenergy));

  if (pbc)
    vol.set_pbc (pbccenter, pbcbox);
  if (transform)
    vol.set_transform (transform);
  if (alignsel)
    vol.set_alignsel (alignsel);
  if (!pbcbox)
  {
  ...
  if (tetrahedral_symm || order1 || order2)
  {
    ...
    vol.set_probe_symmetry (order1, symmaxis1, order2, symmaxis2,
        tetrahedral_symm);
  }
  ...
  if (ret_val < 0)
  {
    Tcl_AppendResult (interp, "\nvolmap: ERROR ", measure_error (ret_val),
        NULL);
  ...
  if (probesel && probesel->molid () != molid)
  {
    ...
    for (i = 0; i < numconf; i++)
    {
      ...
      for (j = 0; j < num_probe_atoms; j++)
      {
        vec_copy (&coor[3 * j], &conformers[i * 3 * num_probe_atoms + 3 * j]);
      }
    }
  }
...
// ExSciTech: ETK - Added Exscitech Volmap!
static int
vmd_volmap_new_fromtype (VMDApp *app, int argc, Tcl_Obj * const objv[],
    Tcl_Interp *interp)
{
  ...
  if (!strcmp (maptype_string, "density"))
    maptype = DENS_MAP;
  else if (!strcmp (maptype_string, "interp"))
    maptype = INTERP_MAP;
  else if (!strcmp (maptype_string, "distance"))
    maptype = DIST_MAP;
  else if (!strcmp (maptype_string, "occupancy"))
    maptype = OCCUP_MAP;
  else if (!strcmp (maptype_string, "mask"))
    maptype = MASK_MAP;
  else if (!strcmp (maptype_string, "coulomb"))
    maptype = CPOTENTIAL_MAP;
  else if (!strcmp (maptype_string, "coulombpotential"))
    maptype = CPOTENTIAL_MAP;
  else if (!strcmp (maptype_string, "coulombmsm"))
    maptype = CPOTENTIALMSM_MAP;
  ...
  for (int arg = 2; arg < argc && !bad_usage; arg++)
  {
    if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-res"))
    {
      ...
      if (resolution <= 0.f)
      {
        Tcl_AppendResult (interp, "volmap: resolution must be positive. (-res)",
            NULL);
        return TCL_ERROR;
      }
      arg++;
    }
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-mol"))
    { // add volmap to mol
      if (arg + 1 >= argc)
        bad_usage = true;
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-checkpoint"))
    {
      ...
      {
        Tcl_AppendResult (interp, "volmap: invalid -checkpoint parameter",
            NULL);
        return TCL_ERROR;
      }
      arg++;
    ...
    else if (accept_usepoints
        && !strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-points"))
    {
      use_point_particles = true;
    }
    else if (accept_cutoff
        && !strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-cutoff"))
    {
      if (arg + 1 >= argc)
        bad_usage = true;
      Tcl_GetDoubleFromObj (interp, objv[arg + 1], &cutoff);
      if (cutoff <= 0.)
      {
        Tcl_AppendResult (interp, "volmap: cutoff must be positive. (-cutoff)",
            NULL);
        return TCL_ERROR;
      }
      arg++;
    }
    else if (accept_radius
        && !strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-radscale"))
    {
      if (arg + 1 >= argc)
        bad_usage = true;
      ...
      {
        Tcl_AppendResult (interp,
            "volmap: radscale must be positive. (-radscale)", NULL);
        return TCL_ERROR;
      }
      arg++;
    }
    else if (accept_weight
        && !strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-weight"))
    {
      if (arg + 1 >= argc)
        bad_usage = true;
      arg_weight = arg + 1;
  ...
  if (arg_combine)
  {
    ...
    else
    {
      Tcl_AppendResult (
          interp,
          "volmap: -combine argument must be: avg, min, \
                                max, stdev, pmf",
          NULL);
      return TCL_ERROR;
    }
  ...
  switch (maptype)
  {
    ...
    case DIST_MAP:
      volcreate = new VolMapCreateDistance (app, sel, (float) resolution,
          (float) cutoff);
      if (!filebase)
      {
    ...
    case OCCUP_MAP:
      volcreate = new VolMapCreateOccupancy (app, sel, (float) resolution,
          use_point_particles);
      if (!filebase)
      {
    ...
    case MASK_MAP:
      volcreate = new VolMapCreateMask (app, sel, (float) resolution,
          (float) cutoff);
      if (!filebase)
      {
    ...
    case CPOTENTIAL_MAP:
      volcreate = new VolMapCreateCoulombPotential (app, sel,
          (float) resolution);
      if (!filebase)
      {
    ...
    case CPOTENTIALMSM_MAP:
      volcreate = new VolMapCreateCoulombPotentialMSM (app, sel,
          (float) resolution);
      if (!filebase)
      {
  ...
  if (volcreate)
  {
    ...
    if (arg_minmax)
      volcreate->set_minmax (float (minmax[0]), float (minmax[1]),
          float (minmax[2]), float (minmax[3]), float (minmax[4]),
          float (minmax[5]));
    ...
    if (export_molecule >= 0)
    {
      ...
      for (i = 0; i < 3; i++)
      {
        ...
        int err = app->molecule_add_volumetric (export_molecule,
            (volmap->name) ? volmap->name : "(no name)", origin, xaxis, yaxis,
            zaxis, volmap->xsize, volmap->ysize, volmap->zsize, volmap->data);
        if (err != 1)
        {
          Tcl_AppendResult (interp,
              "ERROR: export of volmap into molecule was unsuccessful!", NULL);
        }
        else
          volmap->data = NULL; // avoid data being deleted by volmap's destructor (it is now owned by the molecule)
    }
...
static int
vmd_volmap_compare (VMDApp *app, int argc, Tcl_Obj * const objv[],
    Tcl_Interp *interp)
{
  int molid1 = -1;
  int molid2 = -1;
  ...
  {
    Tcl_AppendResult (interp,
        "volmap compare: molecule specified for ouput is invalid. (-mol)",
        NULL);
    return TCL_ERROR;
  }
  ...
  {
    Tcl_AppendResult (interp, "volmap compare: Volmap ID must be positive.",
        NULL);
    return TCL_ERROR;
  }
  if (mapid1 >= mol1->num_volume_data () || mapid2 >= mol2->num_volume_data ())
  {
    Tcl_AppendResult (interp, "volmap compare: Volmap ID does not exist.",
        NULL);
    return TCL_ERROR;
  }
  ...
  for (arg = 5; arg < argc && !bad_usage; arg++)
  {
    if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-interval"))
    {
      ...
      {
        Tcl_AppendResult (interp,
            "volmap compare: histogram interval must be positive. (-interval)",
            NULL);
        return TCL_ERROR;
      }
      arg++;
    ...
    else if (!strcmp (Tcl_GetStringFromObj (objv[arg], NULL), "-numbins"))
    {
      ...
      {
        Tcl_AppendResult (interp,
            "volmap compare: histogram bin size must be positive. (-interval)",
            NULL);
        return TCL_ERROR;
      }
      arg++;
  ...
  {
    Tcl_AppendResult (interp, "volmap compare: maps have different dimensions.",
        NULL);
    return TCL_ERROR;
  }
  ...
  if (norm (vdiff) > 1e-6)
  {
    Tcl_AppendResult (interp, "volmap compare: maps have different origin.",
        NULL);
  }
  ...
  if (norm (vdiff) > 1e-6)
  {
    Tcl_AppendResult (interp, "volmap compare: maps have different x-axis.",
        NULL);
  }DOUBLE_VSUB(vdiff, vol1->yaxis, vol2->yaxis);
  if (norm (vdiff) > 1e-6)
  {
    Tcl_AppendResult (interp, "volmap compare: maps have different y-axis.",
        NULL);
  }DOUBLE_VSUB(vdiff, vol1->zaxis, vol2->zaxis);
  if (norm (vdiff) > 1e-6)
  {
    Tcl_AppendResult (interp, "volmap compare: maps have different z-axis.",
        NULL);
  }
  ...
  float diffrmsd = 0.f;
  if (sqsum)
    rmsd = sqrtf (sqsum / vol1->gridsize ());
  if (sqsumd)
    diffrmsd = sqrtf (sqsumd / numdiff);

  char tmpstr[128];
  msgInfo << "volmap compare" << sendmsg;
  msgInfo << "--------------" << sendmsg;
  msgInfo << "Comparing mol " << molid1 << " -> map " << mapid1 << "/"
      << mol1->num_volume_data () << sendmsg;
  msgInfo << "       to mol " << molid2 << " -> map " << mapid2 << "/"
      << mol2->num_volume_data () << sendmsg;
  ...
  sprintf (tmpstr, "   map1[%d] = %g   map2[%d] = %g   diff = %g", indexmaxdiff,
      vol1->data[indexmaxdiff], indexmaxdiff, vol2->data[indexmaxdiff],
      maxdiff);
  msgInfo << tmpstr << sendmsg;
  ...
  msgInfo << tmpstr << sendmsg;
  sprintf (tmpstr,
      "     diffRMSD = %12.6f  (for the set of differing elements)", diffrmsd);
  msgInfo << tmpstr << sendmsg;
  ...
  if (range)
  {
    ...
    if (sqsum)
      wdiffrmsd = sqrtf (sqsum / wsum);
  }

  sprintf (tmpstr,
      "weighted RMSD = %12.6f  (for the set of differing elements)", wdiffrmsd);
  msgInfo << tmpstr << sendmsg;
  ...
  {
    if (binrmsd[i])
      binrmsd[i] = sqrtf (binrmsd[i] / num[i]);
  }

  // lower boundary of the first bin
  float firstbin = floorf (min2 / float (histinterval)) * histinterval;
  ...
  sprintf (label, "[%g,%g)", (double) firstbin,
      (double) (firstbin + histinterval));
  ...
  Tcl_ListObjAppendElement (interp, objAverage,
      Tcl_NewDoubleObj (!num[0] ? 0 : histo[0] / num[0]));
  ...
  {
    sprintf (label, "[%g,%g)", (double) (firstbin + i * histinterval),
        (double) (firstbin + (i + 1) * histinterval));
    ...
    Tcl_ListObjAppendElement (interp, objAverage,
        Tcl_NewDoubleObj (!num[i] ? 0 : histo[i] / num[i]));
    Tcl_ListObjAppendElement (interp, objMax, Tcl_NewDoubleObj (maxEntry[i]));
    Tcl_ListObjAppendElement (interp, objBinRMSD,
        Tcl_NewDoubleObj (binrmsd[i]));
    msgInfo << tmpstr << sendmsg;
  }
...
int
obj_volmap (ClientData cd, Tcl_Interp *interp, int argc, Tcl_Obj * const objv[])
{
  ...
  if (argc < 2)
  {
    Tcl_SetResult (
        interp,
        (char *) "usage: volmap <command> <args...>\n"
            "\nVolmap Creation:\n"
  ...
  if (argc > 1 && !strupncmp (arg1, "mask", CMDLEN))
    return vmd_volmap_new_fromtype (app, argc - 1, objv + 1, interp);
  if (argc > 1
      && (!strupncmp (arg1, "coulombpotential", CMDLEN)

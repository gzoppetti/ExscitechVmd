 ...
 *	$Revision: 1.313 $	$Date: 2009/07/29 04:44:21 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
int
DrawMolItem::emitstructwarning (void)
{
  ...
  {
    msgErr
        << "Maximum structure display warnings reached, further warnings will be suppressed"
        << sendmsg;
    structwarningcount++;
    return 0;
  }
...
void
DrawMolItem::update_lookups (AtomColor *ac, AtomSel *sel, ColorLookup *lookups)
{
  int i;
...
int
DrawMolItem::get_pbc () const
{
  return cmdList->pbc;
}

void
DrawMolItem::set_pbc_images (int n)
{
  if (n < 1)
    return;
  cmdList->npbc = n;
...
int
DrawMolItem::get_pbc_images () const
{
  return cmdList->npbc;
}

static void
parse_frames (const char *beg, int len, int maxframe, ResizeArray<int>& frames)
{
  if (!len)
  {
...
void
DrawMolItem::set_drawframes (const char *frames)
{
  if (!frames)
    return;
  while (isspace (*frames))
    frames++;
  if (!strncmp (frames, "now", 3) && !strncmp (framesel, "now", 3))
    return;
  delete[] framesel;
...
// Exscitech - Generate your volmappppppp
void
DrawMolItem::generateTriMeshCommand (const VolumetricData * data)
{
  int stepsize = (int) atomRep->get_data (AtomRep::ISOSTEPSIZE);
  fprintf(stderr, "stepsize before %d\n", stepsize);
  if (stepsize >= data->xsize)
    stepsize = (data->xsize - 1);
  if (stepsize >= data->ysize)
    stepsize = (data->ysize - 1);
  if (stepsize >= data->zsize)
    stepsize = (data->zsize - 1);
  if (stepsize < 2)
    stepsize = 1;

  fprintf(stderr, "stepsize after %d\n", stepsize);

  generate_volume_isosurface_trimesh (data,
      atomRep->get_data (AtomRep::SPHERERAD), stepsize);
}
//////////////////////////////// private routines 

void
DrawMolItem::create_cmdlist ()
{
  int i;
  ...
  if (needRegenerate)
  {
        ...
        || atomRep->method () == AtomRep::VOLSLICE)
    {
      ...
      cmdVolTexture.putdata (volumeTexture.getTextureID (),
          volumeTexture.getTextureSize (), volumeTexture.getTextureMap (), v0,
          v1, v2, v3, cmdList);
    }
...
void
DrawMolItem::do_create_cmdlist (void)
{

  float *framepos;
  DispCmdDataBlock dblock;
  int i;
  ...
  if (mol->current ())
  {
    ...
    if (avgsize)
    {
      ...
      const int atomloop = 3 * mol->nAtoms;
      ...
      for (i = 0; i < atomloop; i++) // scale results appropriately
        avg[i] *= rescale;

      framepos = avg; // framepos points to the new average values
    }

    // put current atom coordinates into a datablock
    dblock.putdata (framepos, 3 * mol->nAtoms, cmdList);

    // now put in drawing commands, which index into the above block
    switch (atomRep->method ())
    {
      case AtomRep::LINES:
        draw_lines (framepos, (int) atomRep->get_data (AtomRep::LINETHICKNESS),
            atomRep->get_data (AtomRep::ISOLINETHICKNESS));
        place_picks ();
        break;

      case AtomRep::BONDS:
        draw_bonds (framepos, atomRep->get_data (AtomRep::BONDRAD),
      ...
      case AtomRep::DYNAMICBONDS:
        draw_dynamic_bonds (framepos, atomRep->get_data (AtomRep::BONDRAD),
            (int) atomRep->get_data (AtomRep::BONDRES),
            atomRep->get_data (AtomRep::SPHERERAD));
        break;

      case AtomRep::HBONDS:
        draw_hbonds (framepos, atomRep->get_data (AtomRep::SPHERERAD),
      ...
      case AtomRep::VDW:
        draw_solid_spheres (framepos,
            ...
            atomRep->get_data (AtomRep::SPHERERAD), 0.0);
        place_picks ();
        break;

      case AtomRep::CPK:
        draw_cpk_licorice (framepos, 1, atomRep->get_data (AtomRep::BONDRAD), // bond rad
      ...
      case AtomRep::LICORICE:
        draw_cpk_licorice (framepos, 0, atomRep->get_data (AtomRep::BONDRAD), // bond rad
        (int) atomRep->get_data (AtomRep::BONDRES), // bond res
        atomRep->get_data (AtomRep::SPHERERAD), // scaled VDW rad
        (int) atomRep->get_data (AtomRep::SPHERERES), // sphere res
        (int) atomRep->get_data (AtomRep::LINETHICKNESS), // line thickness
      ...
      case AtomRep::TRACE:
        draw_trace (framepos, atomRep->get_data (AtomRep::BONDRAD),
            (int) atomRep->get_data (AtomRep::BONDRES),
            (int) atomRep->get_data (AtomRep::LINETHICKNESS));
        place_picks ();
        break;
      ...
      case AtomRep::TUBE:
        draw_tube (framepos, atomRep->get_data (AtomRep::BONDRAD),
            (int) atomRep->get_data (AtomRep::BONDRES));
        break;

      case AtomRep::RIBBONS:
        draw_ribbons (framepos, atomRep->get_data (AtomRep::BONDRAD) / 3.0f,
            (int) atomRep->get_data (AtomRep::BONDRES),
            atomRep->get_data (AtomRep::LINETHICKNESS));
        break;

      case AtomRep::NEWRIBBONS:
        draw_ribbons_new (framepos, atomRep->get_data (AtomRep::BONDRAD) / 3.0f,
            (int) atomRep->get_data (AtomRep::BONDRES),
        ...
        place_picks();// XXX add better pick points in the ring traversal code
        break;
      ...
      case AtomRep::NEWCARTOON:
        draw_cartoon_ribbons (framepos,
            (int) atomRep->get_data (AtomRep::BONDRES),
            atomRep->get_data (AtomRep::BONDRAD),
            (float) atomRep->get_data (AtomRep::LINETHICKNESS), 1,
            (int) atomRep->get_data (AtomRep::SPHERERAD));
        break;

      case AtomRep::STRUCTURE:
        draw_structure (framepos, atomRep->get_data (AtomRep::BONDRAD),
            (int) atomRep->get_data (AtomRep::BONDRES),
            (int) atomRep->get_data (AtomRep::LINETHICKNESS));
        break;
      ...
      case AtomRep::ISOSURFACE:
        draw_isosurface ((int) atomRep->get_data (AtomRep::SPHERERES),
            atomRep->get_data (AtomRep::SPHERERAD), //This
            (int) atomRep->get_data (AtomRep::LINETHICKNESS),
            (int) atomRep->get_data (AtomRep::BONDRES),
            (int) atomRep->get_data (AtomRep::ISOSTEPSIZE), //This
            (int) atomRep->get_data (AtomRep::ISOLINETHICKNESS));
        break;

#ifdef VMDORBITALS
        case AtomRep::ORBITAL:
        draw_orbital((int)atomRep->get_data(AtomRep::WAVEFNCTYPE),
      ...
      case AtomRep::DOTTED:
        draw_dotted_spheres (framepos, atomRep->get_data (AtomRep::SPHERERAD),
            (int) atomRep->get_data (AtomRep::SPHERERES));
        place_picks ();
        break;

      case AtomRep::SOLVENT:
        draw_dot_surface (framepos, atomRep->get_data (AtomRep::SPHERERAD),
            (int) atomRep->get_data (AtomRep::SPHERERES),
...
void
DrawMolItem::place_picks (void)
{
  DispCmdPickPointIndexArray cmdPickPointIndexArray;
  cmdPickPointIndexArray.putdata (atomSel->num_atoms, atomSel->selected,
      &atomSel->on[0], cmdList);
}

//////////////////////////////// protected virtual routines

void
DrawMolItem::do_color_changed (int ccat)
{
...
void
DrawMolItem::do_color_rgb_changed (int color)
{
  int ccat;
...
void
DrawMolItem::do_color_scale_changed ()
{
...
int
DrawMolItem::change_color (AtomColor *ac)
{
  if (ac)
  {
    *atomColor = *ac;
...
int
DrawMolItem::representation_index (void)
{
...
void
DrawMolItem::update_pbc_transformations ()
{
...
int
DrawMolItem::change_rep (AtomRep *ar)
{
  if (!ar)
    return FALSE; // XX does this actually occur?
  ...
  if ((atomRep->method () == AtomRep::TUBE
      ...
      && (ar->method () != AtomRep::TUBE && ar->method () != AtomRep::STRUCTURE))
  {
    if (tubearray)
    {
...
int
DrawMolItem::change_sel (const char *cmdStr)
{
  if (!cmdStr)
    return TRUE; // no problem
  if (atomSel->change (cmdStr, mol) == AtomSel::NO_PARSE)
    return FALSE;
...
void
DrawMolItem::force_recalc (int r)
{
  needRegenerate |= r;
}
...
int
DrawMolItem::atom_displayed (int n)
{
...
void
DrawMolItem::draw_lines (float *framepos, int thickness, float cutoff)
{
  ...
  if (cutoff > 0)
  {
    ...
    GridSearchPair *pairlist = vmd_gridsearch1 (framepos, mol->nAtoms,
        atomSel->on, cutoff, 0, mol->nAtoms * 27);
    ...
    for (p = pairlist; p != NULL; p = tmp)
    {
      ...
      if (atom1->atomType == ATOMHYDROGEN && atom2->atomType == ATOMHYDROGEN)
      {
        tmp = p->next;
        free (p);
        continue;
  ...
  for (int i = 0; i < MAXCOLORS; i++)
  {
    ...
    if (cl.num == 0)
      continue; // skip if no bonds of this color
...
void
DrawMolItem::draw_solid_spheres (float * framepos, int res, float radscale,
    float fixrad)
{
  int i;

  // set sphere type, resolution, and shading characteristics
  sprintf (commentBuffer, "MoleculeID: %d ReprID: %d Beginning VDW", mol->id (),
      repNumber);
  cmdCommentX.putdata (commentBuffer, cmdList);
  ...
  { // don't draw zero scaled spheres
    int ind = 0;
    ...
    for (i = 0; i < mol->nAtoms; i++)
    {
      ...
      if (atomSel->on[i])
      {
        ...
        radii.append (radius[i] * radscale + fixrad);
    ...
    if (radii.num () > 0)
    {
      cmdSphereArray.putdata ((float *) &centers[0], (float *) &radii[0],
          (float *) &colors[0], radii.num (), res, cmdList);
    }
  }
}
...
// This allows same code to be used for VDW, CPK, Licorice, etc.
void
DrawMolItem::draw_residue_beads (float * framepos, int sres, float radscale)
{
  int i, resid, numres;
  ...
  {
    cmdSphereArray.putdata ((float *) &centers[0], (float *) &radii[0],
        (float *) &colors[0], radii.num (), sres, cmdList);
  }
#endif
}
...
void
DrawMolItem::draw_dotted_spheres (float * framepos, float srad, int sres)
{
  float radscale;
  int i;
  const float *radius = mol->radius ();
  ...
  { // don't draw zero scaled spheres
    ...
    for (i = 0; i < mol->nAtoms; i++)
    {
      ...
      if (atomSel->on[i])
      {
        ...
        cmdSphereIndex.putdata (3 * i, radius[i] * radscale, cmdList);
      }
    }
  }
}
...
void
DrawMolItem::draw_points (float *framepos, float pointsize)
{
  ...
  if (centers.num () > 0)
  {
    ...
    cmdPointArray.putdata ((float *) &centers[0], (float *) &colors[0],
        pointsize, centers.num () / 3, cmdList);
  }
}

void
DrawMolItem::draw_cpk_licorice (float *framepos, int cpk, float brad, int bres,
    float srad, int sres, int linethickness, float cutoff)
{
  MolAtom *a1;
  int i, j, a2n;
  ...
  sprintf (commentBuffer, "MoleculeID: %d ReprID: %d Beginning CPK", mol->id (),
      repNumber);
  cmdCommentX.putdata (commentBuffer, cmdList);
...
// no gap between them
void
DrawMolItem::draw_bonds (float *framepos, float brad, int bres, float cutoff)
{
  MolAtom *a1, *a2;
    ...
    if (atomSel->on[i])
    {
      ...
      for (j = 0; j < a1->bonds; j++)
      {
        ...
        if (k > i && atomSel->on[k])
        {
          ...
          if (order > 1)
          {
            make_connection (NULL, idouble, p3, kdouble, brad * 1.5f, bres,
                use_cyl);
            if (order > 2)
              make_connection (NULL, itriple, p3, ktriple, brad * 2.0f, bres,
                  use_cyl);
          }
          ...
          if (order > 1)
          {
            make_connection (idouble, p3, kdouble, NULL, brad * 1.5f, bres,
                use_cyl);
            if (order > 2)
              make_connection (itriple, p3, ktriple, NULL, brad * 2.0f, bres,
                  use_cyl);
          }
        } // found k atom
...
void
DrawMolItem::make_connection (float *prev, float *start, float *end,
...
void
DrawMolItem::draw_spline_curve (int num, float *coords, int *idx, int use_cyl,
    float b_rad, int b_res)
{
        ...
        || (idx[loop + 1] >= 0 && atomSel->on[idx[loop + 1]]))
    {
                  ...
                  && atomSel->on[idx[loop + 1]])))
      {
        ...
        { // no there wasn't
          ...
          make_connection (NULL, final[0], final[1], final[2], b_rad, b_res,
              use_cyl);
        }
        else
        {
          ...
          make_connection (prev[0], prev[1], final[0], final[1], b_rad, b_res,
              use_cyl);
          make_connection (prev[1], final[0], final[1], final[2], b_rad, b_res,
              use_cyl);
        }
        make_connection (final[0], final[1], final[2], final[3], b_rad, b_res,
            use_cyl);
        make_connection (final[1], final[2], final[3], final[4], b_rad, b_res,
            use_cyl);
                  ...
                  && atomSel->on[idx[loop]])))
      {
        ...
        make_connection (final[2], final[3], final[4], final[5], b_rad, b_res,
            use_cyl);
        make_connection (final[3], final[4], final[5], final[6], b_rad, b_res,
            use_cyl);
        last_loop = loop;
      }
  ...
  {
    DispCmdPickPointIndexArray pickPointIndexArray;
    pickPointIndexArray.putdata (pickpointindices.num (), &pickpointindices[0],
        cmdList);
  }
}

void
DrawMolItem::generate_tubearray ()
{
  if (tubearray)
  {
    for (int i = 0; i < tubearray->num (); i++)
...
void
DrawMolItem::draw_tube (float *framepos, float b_rad, int b_res)
{
  ...
  int use_cyl = FALSE;
  if (b_res <= 2 || b_rad < 0.01)
  { // then going to do lines
    append (DMATERIALOFF);
    cmdLineType.putdata (SOLIDLINE, cmdList);
    cmdLineWidth.putdata (2, cmdList);
  }
  ...
  for (int i = 0; i < tubearray->num (); i++)
  {
    ...
    draw_spline_curve (num - 4, coords + 6, &(indxlist[0]) + 2, use_cyl, b_rad,
        b_res);
    delete[] coords;
  }
}
...
void
DrawMolItem::draw_hbonds (float *framepos, float maxangle, int thickness,
    float maxdist)
{
  unsigned int i, k, natoms;
  float donortoH[3], Htoacceptor[3];
  float cosmaxangle2 = cosf (maxangle);
  ...
  GridSearchPair *pairlist = vmd_gridsearch1 (framepos, natoms, onlist, maxdist,
      0, natoms * 27);
  GridSearchPair *p, *tmp;
  for (p = pairlist; p != NULL; p = tmp)
  {
    ...
    if (!a2->bonded (p->ind1))
    {
      unsigned int b1 = a1->bonds;
      unsigned int b2 = a2->bonds;
...
void
DrawMolItem::draw_dynamic_bonds (float *framepos, float brad, int bres,
    float maxdist)
{
...
#if 0
      if (cutoff < 0)
      { // Do atom-specific distance check
...
static void
least_squares (int n, const float *x, float *a, float *b)
{
  float sum = 0;
  int i;
  for (i = 0; i < n; i++)
...
void
DrawMolItem::draw_alpha_helix_cylinders (ResizeArray<float> &x,
    ResizeArray<float> &y, ResizeArray<float> &z, ResizeArray<int> &atom_on,
    int *color, float bond_rad, int bond_res, float *start_coord,
    float *end_coord)
{
...
void
DrawMolItem::draw_beta_sheet (ResizeArray<float> &x, ResizeArray<float> &y,
    ResizeArray<float> &z, ResizeArray<int> &atom_on, int *color,
    float ribbon_width, float *start_coord, float *end_coord)
{
  ResizeArray<int> pickpointindices;
  ...
  for (i = 1; i < num - 1; i++)
  {
    ...
    if (i % 2)
    { // flip every other normal so they are aligned
    ...
    if (atom_on[i] >= 0)
    {
      ...
      if (ribbon_width != 0)
      {
        ...
        if (i == 0 || dot > .9 || dot < -.9)
        {
          ...
          cmdTriangle.putdata (ur[0], ul[1], ur[1], perps + 3 * i,
              perps + 3 * i + 3, perps + 3 * i + 3, cmdList);
          cmdTriangle.putdata (lr[0], ll[0], ll[1], // bottom
              perps + 3 * i, perps + 3 * i, perps + 3 * i + 3, cmdList);
          cmdTriangle.putdata (lr[0], ll[1], lr[1], perps + 3 * i,
              perps + 3 * i + 3, perps + 3 * i + 3, cmdList);

          // and the other sides
          cmdTriangle.putdata (ul[0], ll[0], ll[1], norms + 3 * i,
              norms + 3 * i, norms + 3 * i + 3, cmdList);
          cmdTriangle.putdata (ul[0], ll[1], ul[1], norms + 3 * i,
              norms + 3 * i + 3, norms + 3 * i + 3, cmdList);
          cmdTriangle.putdata (ur[0], lr[0], lr[1], norms + 3 * i,
              norms + 3 * i, norms + 3 * i + 3, cmdList);
          cmdTriangle.putdata (ur[0], lr[1], ur[1], norms + 3 * i,
              norms + 3 * i + 3, norms + 3 * i + 3, cmdList);
        }
        else
        {
          ...
          cmdTriangle.putdata (ur[0], ul2, ur2, perps + 3 * i, new_perp,
              new_perp, cmdList);
          cmdTriangle.putdata (lr[0], ll[0], ll2, // bottom
              perps + 3 * i, perps + 3 * i, new_perp, cmdList);
          cmdTriangle.putdata (lr[0], ll2, lr2, perps + 3 * i, new_perp,
              new_perp, cmdList);

          // and the other sides
          cmdTriangle.putdata (ul[0], ll[0], ll2, norms + 3 * i, norms + 3 * i,
              new_norm, cmdList);
          cmdTriangle.putdata (ul[0], ll2, ul2, norms + 3 * i, new_norm,
              new_norm, cmdList);
          cmdTriangle.putdata (ur[0], lr[0], lr2, norms + 3 * i, norms + 3 * i,
              new_norm, cmdList);
          cmdTriangle.putdata (ur[0], lr2, ur2, norms + 3 * i, new_norm,
              new_norm, cmdList);

          // get the 1/2 half
          cmdTriangle.putdata (ur2, ul2, ul[1], // top
              new_perp, new_perp, perps + 3 * i + 3, cmdList);
          cmdTriangle.putdata (ur2, ul[1], ur[1], new_perp, perps + 3 * i + 3,
              perps + 3 * i + 3, cmdList);
          cmdTriangle.putdata (lr2, ll2, ll[1], // bottom
              new_perp, new_perp, perps + 3 * i + 3, cmdList);
          cmdTriangle.putdata (lr2, ll[1], lr[1], new_perp, perps + 3 * i + 3,
              perps + 3 * i + 3, cmdList);

          // and the other sides
          cmdTriangle.putdata (ul2, ll2, ll[1], new_norm, new_norm,
              norms + 3 * i + 3, cmdList);
          cmdTriangle.putdata (ul2, ll[1], ul[1], new_norm, norms + 3 * i + 3,
              norms + 3 * i + 3, cmdList);
          cmdTriangle.putdata (ur2, lr2, lr[1], new_norm, new_norm,
              norms + 3 * i + 3, cmdList);
          cmdTriangle.putdata (ur2, lr[1], ur[1], new_norm, norms + 3 * i + 3,
              norms + 3 * i + 3, cmdList);
        }
...
void
DrawMolItem::draw_structure (float *framepos, float brad, int bres,
    int linethickness)
{
  ...
  for (frag = 0; frag < mol->pfragList.num (); frag++)
  {
    ...
    for (int resindex = 0; resindex < num; resindex++)
    {
      ...
      int residue_is_helix = (ss == SS_HELIX_ALPHA || ss == SS_HELIX_3_10
          || ss == SS_HELIX_PI);
      ...
      if (!residue_is_helix || resindex == num - 1)
      {
        ...
        else
        {
          ...
          draw_alpha_helix_cylinders (x, y, z, atom_on, atomColor->color, brad,
              bres, ends[0], ends[1]);
      ...
      if (b_start != -1 && (!is_beta || resindex == num - 1))
      {
        ...
        else
        {
          ...
          draw_beta_sheet (x, y, z, atom_on, atomColor->color,
              linethickness / 5.0f, ends[0], ends[1]);
  ...
  // Anyway, when I return I swap everything back
  {
    ...
    {
      if (!atomSel->on[i])
      { // mask the coil against the selected atoms
        on[i] = 0;
      }
    }
    ...
    // make the tube
    draw_tube (framepos, brad / 7.0f, int (float (bres) * 2.0f / 3.0f + 0.5f));

 ...
 *      $Revision: 1.445 $      $Date: 2009/07/20 15:33:05 $
 *
 ***************************************************************************/
...
int
VMDApp::VMDinit (int argc, char **argv, const char *displaytype,
    int *displayLoc, int *displaySize)
{
  ...
  if (!strcmp (displaytype, "WIN") || !strcmp (displaytype, "OPENGL"))
  {
    ...
    display = new Exscitech::QtOpenGLDisplayDevice (this, dsiz, dloc);
#elif defined(VMDFLTKOPENGL)
...
int
VMDApp::remove_menu (const char *name)
{
...
void
VMDApp::menu_add_extension (const char *shortname, const char *menu_path)
{
  commandQueue->runcommand (new CmdMenuExtensionAdd (shortname, menu_path));
}

void
VMDApp::menu_remove_extension (const char *shortname)
{
  commandQueue->runcommand (new CmdMenuExtensionRemove (shortname));
}

const char *
VMDApp::menu_name (int i)
{
  return menulist->name (i);
}

int
VMDApp::menu_status (const char *name)
{
  int id = menulist->typecode (name);
  if (id == -1)
    return 0;
...
int
VMDApp::menu_location (const char *name, int &x, int &y)
{
  int id = menulist->typecode (name);
  if (id == -1)
    return 0;
...
int
VMDApp::menu_show (const char *name, int on)
{
  int id = menulist->typecode (name);
  if (id == -1)
    return 0;
...
int
VMDApp::menu_move (const char *name, int x, int y)
{
  int id = menulist->typecode (name);
  if (id == -1)
    return 0;
...
int
VMDApp::menu_select_mol (const char *name, int molno)
{
  int id = menulist->typecode (name);
  if (id == -1)
    return 0;
...
int
VMDApp::VMDupdate (int check_for_events)
{
...
void
VMDApp::VMDexit (const char *exitmsg, int exitcode, int pauseseconds)
{
#if defined(VMDTKCON)
...
void
VMDApp::show_stride_message ()
{
  if (stride_firsttime)
  {
    stride_firsttime = 0;
    msgInfo << "In any publication of scientific results based in part or\n"
...
char *
VMDApp::vmd_choose_file (const char *title, const char *extension,
...
// file renderer API
int
VMDApp::filerender_num ()
{
  return fileRenderList->num ();
}
const char *
VMDApp::filerender_name (int n)
{
  return fileRenderList->name (n);
}
int
VMDApp::filerender_valid (const char *method)
{
  return (fileRenderList->find (method) != NULL);
}
int
VMDApp::filerender_has_antialiasing (const char *method)
{
  return (fileRenderList->has_antialiasing (method));
}
int
VMDApp::filerender_aasamples (const char *method, int aasamples)
{
  return fileRenderList->aasamples (method, aasamples);
}
int
VMDApp::filerender_aosamples (const char *method, int aosamples)
{
  return fileRenderList->aosamples (method, aosamples);
}
int
VMDApp::filerender_imagesize (const char *method, int *w, int *h)
{
  if (!w || !h)
    return FALSE;
...
int
VMDApp::filerender_has_imagesize (const char *method)
{
  return fileRenderList->has_imagesize (method);
}
int
VMDApp::filerender_aspectratio (const char *method, float *aspect)
{
  if (!aspect)
    return FALSE;
...
int
VMDApp::filerender_numformats (const char *method)
{
  return fileRenderList->numformats (method);
}
const char *
VMDApp::filerender_get_format (const char *method, int i)
{
  return fileRenderList->format (method, i);
}
const char *
VMDApp::filerender_cur_format (const char *method)
{
  return fileRenderList->format (method);
}
int
VMDApp::filerender_set_format (const char *m, const char *fmt)
{
  return fileRenderList->set_format (m, fmt);
}

int
VMDApp::filerender_render (const char *m, const char *f, const char *e)
{
...
const char *
VMDApp::filerender_option (const char *m, const char *o)
{
...
int
VMDApp::scene_rotate_by (float angle, char ax, float incr)
{
  if (ax < 'x' || ax > 'z')
    return FALSE; // failed
  rocker->stop_rocking ();
...
int
VMDApp::scene_rotate_to (float angle, char ax)
{
  if (ax < 'x' || ax > 'z')
    return FALSE; // failed
  rocker->stop_rocking ();
...
int
VMDApp::scene_rotate_by (const float *m)
{
  Matrix4 mat (m);
...
int
VMDApp::scene_rotate_to (const float *m)
{
  Matrix4 mat (m);
...
int
VMDApp::scene_translate_by (float x, float y, float z)
{
...
int
VMDApp::scene_translate_to (float x, float y, float z)
{
...
void
VMDApp::scene_resetview_newmoldata ()
{
#if 1
...
int
VMDApp::scene_rock (char ax, float step, int nsteps)
{
  if (ax < 'x' || ax > 'z')
    return FALSE; // failed
...
int
VMDApp::animation_num_dirs ()
{
  return Animation::ANIM_TOTAL_DIRS;
}

const char *
VMDApp::animation_dir_name (int i)
{
  if (i < 0 || i >= Animation::ANIM_TOTAL_DIRS)
    return NULL;
  return animationDirName[i];
}

int
VMDApp::animation_set_dir (int d)
{
...
int
VMDApp::animation_num_styles ()
{
  return Animation::ANIM_TOTAL_STYLES;
}

const char *
VMDApp::animation_style_name (int i)
{
  if (i < 0 || i >= Animation::ANIM_TOTAL_STYLES)
    return NULL;
  return animationStyleName[i];
}

int
VMDApp::animation_set_style (int s)
{
...
int
VMDApp::animation_set_frame (int frame)
{
...
int
VMDApp::animation_set_stride (int stride)
{
  anim->skip (stride);
...
int
VMDApp::animation_set_speed (float speed)
{
  anim->speed (speed);
...
const char *
VMDApp::filerender_default_option (const char *m)
{
  FileRenderer *ren = fileRenderList->find (m);
  if (!ren)
  {
...
const char *
VMDApp::filerender_default_filename (const char *m)
{
  FileRenderer *ren = fileRenderList->find (m);
  if (!ren)
  {
...
vmdplugin_t *
VMDApp::get_plugin (const char *type, const char *name)
{
  if (!pluginMgr)
    return NULL;
  if (!type || !name)
  ...
  if (pluginMgr->plugins (p, type, name))
  {
    ...
    for (int i = 1; i < p.num (); i++)
    {
      ...
      if (curplugin->majorv > plugin->majorv || (curplugin->majorv
          == plugin->majorv && curplugin->minorv > plugin->minorv))
        plugin = curplugin;
    }
  }
  return plugin;
...
int
VMDApp::list_plugins (PluginList &p, const char *type)
{
  if (!pluginMgr)
    return 0;
...
int
VMDApp::plugin_dlopen (const char *filename)
{
  if (!pluginMgr)
  {
...
void
VMDApp::plugin_update ()
{
  commandQueue->runcommand (new CmdPluginUpdate);
}

void
VMDApp::display_update_on (int ison)
{
  UpdateDisplay = ison;
}

int
VMDApp::display_update_status ()
{
  return (UpdateDisplay != 0);
}

void
VMDApp::display_update ()
{
...
void
VMDApp::display_update_ui ()
{
  VMDupdate (VMD_CHECK_EVENTS);
}

int
VMDApp::num_color_categories ()
{
  return scene->num_categories ();
}
const char *
VMDApp::color_category (int n)
{
  return scene->category_name (n);
}

int
VMDApp::color_add_item (const char *cat, const char *name, const char *defcolor)
{
  int init_color = scene->color_index (defcolor);
  if (init_color < 0)
  {
    msgErr << "Cannot add color item: invalid color name '" << defcolor << "'"
        << sendmsg;
    return FALSE;
  }
...
int
VMDApp::num_color_category_items (const char *category)
{
...
const char *
VMDApp::color_category_item (const char *category, int n)
{
  int colCatIndex = scene->category_index (category);
  if (colCatIndex < 0)
    return 0;
...
int
VMDApp::num_regular_colors ()
{
  return REGCLRS;
}
const char *
VMDApp::color_name (int n)
{
  return scene->color_name (n);
}
int
VMDApp::color_index (const char *color)
{
  if (!color)
    return -1;
...
int
VMDApp::color_value (const char *colorname, float *r, float *g, float *b)
{
...
int
VMDApp::color_default_value (const char *colorname, float *r, float *g,
    float *b)
{
  int colIndex = color_index (colorname);
  if (colIndex < 0)
...
const char *
VMDApp::color_mapping (const char *category, const char *item)
{
...
const char *
VMDApp::color_get_restype (const char *resname)
{
...
int
VMDApp::color_set_restype (const char *resname, const char *restype)
{
  int cat = moleculeList->colorCatIndex[MLCAT_RESTYPES];
  int ind = scene->category_item_index (cat, restype);
  if (ind < 0)
    return FALSE; // nonexistent restype category
...
int
VMDApp::colorscale_info (float *mid, float *min, float *max)
{
  scene->colorscale_value (mid, min, max);
  return 1;
}
int
VMDApp::num_colorscale_methods ()
{
  return scene->num_colorscale_methods ();
}
int
VMDApp::colorscale_method_current ()
{
  return scene->colorscale_method ();
}
const char *
VMDApp::colorscale_method_name (int n)
{
  if (n < 0 || n >= scene->num_colorscale_methods ())
    return NULL;
  return scene->colorscale_method_name (n);
}
int
VMDApp::colorscale_method_index (const char *method)
{
...
int
VMDApp::get_colorscale_colors (int whichScale, float min[3], float mid[3],
    float max[3])
{
...
int
VMDApp::set_colorscale_colors (int whichScale, const float min[3],
    const float mid[3], const float max[3])
{
  if (scene->set_colorscale_colors (whichScale, min, mid, max))
  {
    commandQueue->runcommand (
        new CmdColorScaleColors (scene->colorscale_method_name (whichScale),
            mid, min, max));
    return TRUE;
  }
  return FALSE;
...
int
VMDApp::color_changename (const char *category, const char *colorname,
    const char *color)
{

  if (!category || !colorname || !color)
    return 0;
...
int
VMDApp::color_get_from_name (const char *category, const char *colorname,
    const char **color)
{
...
int
VMDApp::color_changevalue (const char *color, float r, float g, float b)
{
...
int
VMDApp::colorscale_setvalues (float mid, float min, float max)
{
...
int
VMDApp::colorscale_setmethod (int method)
{
  if (method < 0 || method >= scene->num_colorscale_methods ())
    return 0;
  scene->set_colorscale_method (method);
  commandQueue->runcommand (
      new CmdColorScaleMethod (scene->colorscale_method_name (method)));
  return 1;
}

int
VMDApp::logfile_read (const char *path)
{
...
VMDApp::save_state ()
{
  char *file = vmd_choose_file ("Enter filename to save current VMD state:", // Title
      "*.vmd", // extension
      "VMD files", // label
      1 // do_save
      );
...
int
VMDApp::num_molecules ()
{
  return moleculeList->num ();
}

int
VMDApp::molecule_new (const char *name, int natoms)
{
...
const char *
VMDApp::guess_filetype (const char *filename)
{
  ...
  if (!ext)
  {
    ...
    msgWarn << "Unable to ascertain filetype from filename '" << filename
        << "'; assuming pdb." << sendmsg;
    return "pdb";
  }
  ext++;
  char *s = strdup (ext);
...
int
VMDApp::molecule_load (int molid, const char *filename, const char *filetype,
    const FileSpec *spec)
{
  ...
  if (!p)
    p = get_plugin ("mol file converter", filetype);
  if (!p)
  {
  ...
  sprintf (specstr, "first %d last %d step %d filebonds %d autobonds %d",
      spec->first, spec->last, spec->stride, spec->autobonds, spec->filebonds);
  ...
  if (plugin->can_read_structure ())
  {
    if (!newmol->has_structure ())
    {
      ...
      int rc =
          plugin->read_structure (newmol, spec->filebonds, spec->autobonds);
  ...
  if (plugin->can_read_timesteps () || plugin->can_read_qm_timestep ())
  {
    ...
    if (!newmol->nAtoms)
    {
      msgErr << "Some frames from file '" << filename
          << "' could not be loaded" << sendmsg;
      msgErr << "because the number of atoms could not be determined.  Load a"
          << sendmsg;
      msgErr << "structure file first, then try loading this file again."
          << sendmsg;
    }
    else
    {

      CoorPluginData *data = new CoorPluginData (filename, newmol, plugin, 1,
          spec->first, spec->stride, spec->last);
  ...
  if (first_structure)
  {
    ...
    moleculeList->set_representation (
        (char *) moleculeList->default_representation ());
  ...
  commandQueue->runcommand (
      new CmdMolLoad (original_molid, filename, filetype, spec));
  return molid;
}

int
VMDApp::molecule_savetrajectory (int molid, const char *fname,
    const char *type, const FileSpec *spec)
{
  ...
  if (natoms > 0)
  {
    ...
    {
      msgInfo
          << "Save Trajectory: 0 atoms in molecule or selection; no coordinates written."
          << sendmsg;
      return -1;
    }
  }
  ...
  if (!p)
    p = get_plugin ("mol file converter", type);
    ...
    if (plugin->can_write_volumetric ())
    {
      for (int i = 0; i < spec->nvolsets; i++)
      {
        if (plugin->write_volumetric (newmol, spec->setids[i])
            != MOLFILE_SUCCESS)
        {
          msgErr << "Failed to write volume set " << spec->setids[i] << sendmsg;
        }
      }
    }
    else
  ...
  else if (waitfor > 0)
  {
    for (int i = 0; i < waitfor; i++)
    {
      if (data->next (newmol) == CoorData::DONE)
        break;
      numwritten++;
    }
  ...
  commandQueue->runcommand (
      new CmdAnimWriteFile (molid, fname, type, first, last, stride));
  return numwritten;
}

int
VMDApp::molecule_deleteframes (int molid, int first, int last, int stride)
{
  ...
  if (stride == -1)
    stride = 0; //delete all frames in range
  if (stride < 0)
  {
    msgErr << "Invalid stride: " << stride << sendmsg;
    return 0;
...
int
VMDApp::molecule_id (int i)
{
  if (i < 0 || i >= num_molecules ())
    return -1;
...
int
VMDApp::molecule_valid_id (int molid)
{
  return (moleculeList->mol_from_id (molid) != NULL);
}
int
VMDApp::molecule_cancel_io (int molid)
{
...
int
VMDApp::molecule_delete (int molid)
{
  if (moleculeList->del_molecule (molid))
  {
    ...
    commandQueue->runcommand (
        new MoleculeEvent (molid, MoleculeEvent::MOL_DELETE));
...
int
VMDApp::molecule_delete_all (void)
{
  int i, nummols, rc;
  int *molidlist;
...
int
VMDApp::molecule_activate (int molid, int onoff)
{
...
int
VMDApp::molecule_is_active (int molid)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
...
int
VMDApp::molecule_fix (int molid, int onoff)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
...
int
VMDApp::molecule_is_fixed (int molid)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
...
int
VMDApp::molecule_display (int molid, int onoff)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
...
int
VMDApp::molecule_is_displayed (int molid)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
...
int
VMDApp::molecule_make_top (int molid)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
...
const char *
VMDApp::molrep_get_style (int molid, int repid)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return NULL;
...
int
VMDApp::molrep_set_style (int molid, int repid, const char *style)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
  if (!moleculeList->change_repmethod (repid, ind, (char *) style))
    return 0;
  commandQueue->runcommand (
...
const char *
VMDApp::molrep_get_color (int molid, int repid)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return NULL;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_set_color (int molid, int repid, const char *color)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
  if (!moleculeList->change_repcolor (repid, ind, (char *) color))
    return 0;
  commandQueue->runcommand (
...
const char *
VMDApp::molrep_get_selection (int molid, int repid)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return NULL;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_set_selection (int molid, int repid, const char *selection)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return FALSE;
  if (!moleculeList->change_repsel (repid, ind, selection))
    return FALSE;
  commandQueue->runcommand (
      new CmdMolChangeRepItem (repid, molid, CmdMolChangeRepItem::SEL,
          selection));
  return TRUE;
}
int
VMDApp::molrep_numselected (int molid, int repid)
{
  if (repid >= num_molreps (molid))
    return -1;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
  if (item == NULL)
    return -1;
...
const char *
VMDApp::molrep_get_material (int molid, int repid)
{
  if (repid >= num_molreps (molid))
    return NULL;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_set_material (int molid, int repid, const char *material)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
  if (!moleculeList->change_repmat (repid, ind, (char *) material))
    return 0;
  commandQueue->runcommand (
...
const char *
VMDApp::molecule_get_style ()
{
  return moleculeList->representation ();
}
int
VMDApp::molecule_set_style (const char *style)
{
...
const char *
VMDApp::molecule_get_color ()
{
  return moleculeList->color ();
}
int
VMDApp::molecule_set_color (const char *color)
{
...
const char *
VMDApp::molecule_get_selection ()
{
  return moleculeList->selection ();
}
int
VMDApp::molecule_set_selection (const char *selection)
{
...
const char *
VMDApp::molecule_get_material ()
{
  return moleculeList->material ();
}
int
VMDApp::molecule_set_material (const char *material)
{
...
int
VMDApp::molecule_addrep (int molid)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
  if (!moleculeList->add_rep (ind))
    return 0;
  commandQueue->runcommand (new CmdMolAddRep (molid));
  return 1;
}
int
VMDApp::molecule_modrep (int molid, int repid)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
  if (!moleculeList->change_rep (repid, ind))
    return 0;
  commandQueue->runcommand (new CmdMolChangeRep (molid, repid));
  return 1;
}
int
VMDApp::molrep_delete (int molid, int repid)
{
  int ind = moleculeList->mol_index_from_id (molid);
  if (ind < 0)
    return 0;
  if (!moleculeList->del_rep (repid, ind))
    return 0;
...
int
VMDApp::molrep_get_selupdate (int molid, int repid)
{
  if (repid >= num_molreps (molid))
    return 0;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_set_selupdate (int molid, int repid, int onoff)
{
  if (repid >= num_molreps (molid))
    return 0;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_set_colorupdate (int molid, int repid, int onoff)
{
  if (repid >= num_molreps (molid))
    return 0;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_get_colorupdate (int molid, int repid)
{
  if (repid >= num_molreps (molid))
    return 0;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_set_smoothing (int molid, int repid, int n)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return FALSE;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
  if (item->get_smoothing () == n)
    return TRUE;
...
int
VMDApp::molrep_get_smoothing (int molid, int repid)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return -1;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_set_pbc (int molid, int repid, int pbc)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return FALSE;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_get_pbc (int molid, int repid)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return FALSE;
  const DrawMolItem *item =
      moleculeList->mol_from_id (molid)->component (repid);
  return item->get_pbc ();
}

int
VMDApp::molrep_set_pbc_images (int molid, int repid, int n)
{
  if (n < 1)
    return FALSE;
  if (repid < 0 || repid >= num_molreps (molid))
...
int
VMDApp::molrep_get_pbc_images (int molid, int repid)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return -1;
  const DrawMolItem *item =
...
int
VMDApp::molecule_set_dataset_flag (int molid, const char *dataflagstr,
    int setval)
{
  ...
  {
    dataflag = BaseMolecule::INSERTION | BaseMolecule::OCCUPANCY
        | BaseMolecule::BFACTOR | BaseMolecule::MASS | BaseMolecule::CHARGE
        | BaseMolecule::RADIUS | BaseMolecule::ALTLOC
        | BaseMolecule::ATOMICNUMBER | BaseMolecule::BONDS
        | BaseMolecule::BONDORDERS | BaseMolecule::BONDTYPES
        | BaseMolecule::ANGLES | BaseMolecule::ANGLETYPES
        | BaseMolecule::CTERMS;
  }
...
int
VMDApp::molecule_reanalyze (int molid)
{
  Molecule *m = moleculeList->mol_from_id (molid);
  if (!m)
    return 0;
  ...
  m->force_recalc (
      DrawMolItem::COL_REGEN | DrawMolItem::MOL_REGEN | DrawMolItem::SEL_REGEN);
...
int
VMDApp::molecule_bondsrecalc (int molid)
{
  Molecule *m = moleculeList->mol_from_id (molid);
  if (!m)
    return 0;
...
int
VMDApp::molecule_ssrecalc (int molid)
{
  Molecule *m = moleculeList->mol_from_id (molid);
  if (!m)
    return FALSE;
...
int
VMDApp::molecule_numatoms (int molid)
{
  Molecule *m = moleculeList->mol_from_id (molid);
  if (!m)
    return -1;
...
int
VMDApp::molecule_numframes (int molid)
{
  Molecule *m = moleculeList->mol_from_id (molid);
  if (!m)
    return -1;
...
int
VMDApp::molecule_frame (int molid)
{
  Molecule *m = moleculeList->mol_from_id (molid);
  if (!m)
    return -1;
...
int
VMDApp::molecule_dupframe (int molid, int frame)
{
  Molecule *m = moleculeList->mol_from_id (molid);
  if (!m)
  {
...
const char *
VMDApp::molecule_name (int molid)
{
  Molecule *m = moleculeList->mol_from_id (molid);
  if (!m)
    return NULL;
...
int
VMDApp::molecule_rename (int molid, const char *newname)
{
  Molecule *m = moleculeList->mol_from_id (molid);
  if (!m)
    return 0;
  ...
  commandQueue->runcommand (
      new MoleculeEvent (molid, MoleculeEvent::MOL_RENAME));
  return 1;
}

int
VMDApp::molecule_add_volumetric (int molid, const char *dataname,
...
void
VMDApp::set_mouse_callbacks (int on)
{
  mouse->set_callbacks (on);
}

void
VMDApp::set_mouse_rocking (int on)
{
...
int
VMDApp::num_clipplanes ()
{
  return VMD_MAX_CLIP_PLANE;
}
int
VMDApp::molrep_get_clipplane (int molid, int repid, int clipid, float *center,
...
int
VMDApp::molrep_set_clipcenter (int molid, int repid, int clipid,
    const float *center)
{
  Molecule *mol = moleculeList->mol_from_id (molid);
  if (!mol)
...
int
VMDApp::molrep_set_clipnormal (int molid, int repid, int clipid,
    const float *normal)
{
  Molecule *mol = moleculeList->mol_from_id (molid);
  if (!mol)
...
int
VMDApp::molrep_set_clipcolor (int molid, int repid, int clipid,
    const float *color)
{
  Molecule *mol = moleculeList->mol_from_id (molid);
  if (!mol)
...
int
VMDApp::molrep_set_clipstatus (int molid, int repid, int clipid, int mode)
{
  Molecule *mol = moleculeList->mol_from_id (molid);
  if (!mol)
    return 0;
...
const char *
VMDApp::molrep_get_name (int molid, int repid)
{
  Molecule *mol = moleculeList->mol_from_id (molid);
  if (!mol)
    return NULL;
...
int
VMDApp::molrep_get_by_name (int molid, const char *name)
{
  Molecule *mol = moleculeList->mol_from_id (molid);
  if (!mol)
    return -1;
...
int
VMDApp::molrep_get_scaleminmax (int molid, int repid, float *min, float *max)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return FALSE;
  const DrawMolItem *item =
...
int
VMDApp::molrep_set_scaleminmax (int molid, int repid, float min, float max)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return FALSE;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_reset_scaleminmax (int molid, int repid)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return FALSE;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
int
VMDApp::molrep_set_drawframes (int molid, int repid, const char *framesel)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return FALSE;
  if (!framesel)
...
const char *
VMDApp::molrep_get_drawframes (int molid, int repid)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return NULL;
  const DrawMolItem *item =
...
int
VMDApp::molrep_show (int molid, int repid, int onoff)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return FALSE;
...
int
VMDApp::molrep_is_shown (int molid, int repid)
{
  if (repid < 0 || repid >= num_molreps (molid))
    return FALSE;
  DrawMolItem *item = moleculeList->mol_from_id (molid)->component (repid);
...
//
int
VMDApp::imd_connect (int molid, const char *host, int port)
{
#ifdef VMDIMD
  Molecule *mol = moleculeList->mol_from_id(molid);
  if (!mol) return 0;
...
int
VMDApp::imd_connected (int molid)
{
#ifdef VMDIMD
  if (!imdMgr) return 0;
...
int
VMDApp::imd_sendforces (int num, const int *ind, const float *forces)
{
#ifdef VMDIMD
  if (!imdMgr) return 0;
...
int
VMDApp::imd_disconnect (int molid)
{
#ifdef VMDIMD
...
void
VMDApp::display_set_screen_height (float ht)
{
  if (!display)
    return;
...
float
VMDApp::display_get_screen_height ()
{
  if (!display)
    return 0.0f;
...
//Exscitech
float
VMDApp::display_get_screen_width ()
{
  if (!display)
    return 0.0f;
...
void
VMDApp::display_set_screen_distance (float d)
{
  if (!display)
    return;
...
float
VMDApp::display_get_screen_distance ()
{
  if (!display)
    return 0.0f;
...
void
VMDApp::display_set_position (int x, int y)
{
  if (display)
...
#endif
void
VMDApp::display_set_size (int w, int h)
{
  if (display)
  {
...
void
VMDApp::display_get_size (int *w, int *h)
{
  if (display)
  {
    *w = display->xSize;
...
int
VMDApp::display_set_stereo (const char *mode)
{
  if (!mode)
    return FALSE;
...
int
VMDApp::display_set_cachemode (const char *mode)
{
  if (!mode)
    return FALSE;
...
int
VMDApp::display_set_rendermode (const char *mode)
{
  if (!mode)
    return FALSE;
...
int
VMDApp::display_set_eyesep (float sep)
{
  if (sep < 0)
    return FALSE;
...
int
VMDApp::display_set_focallen (float flen)
{
  if (!display->set_eye_dist (flen))
    return FALSE;
...
int
VMDApp::display_set_projection (const char *proj)
{
  if (!display->set_projection (proj))
    return FALSE;
...
int
VMDApp::display_projection_is_perspective (void)
{
...
int
VMDApp::display_set_aa (int onoff)
{
  if (!display->aa_available ())
    return FALSE;
  if (onoff)
    display->aa_on ();
  else
...
int
VMDApp::display_set_depthcue (int onoff)
{
  if (!display->cueing_available ())
    return FALSE;
  if (onoff)
    display->cueing_on ();
  else
...
int
VMDApp::display_set_culling (int onoff)
{
  if (!display->culling_available ())
    return FALSE;
  if (onoff)
    display->culling_on ();
  else
...
int
VMDApp::display_set_fps (int onoff)
{
  if (onoff)
    fps->on ();
  else
...
int
VMDApp::display_set_background_mode (int mode)
{
...
int
VMDApp::display_set_nearclip (float amt, int isdelta)
{
  if (isdelta)
  {
...
int
VMDApp::display_set_farclip (float amt, int isdelta)
{
  if (isdelta)
  {
...
int
VMDApp::stage_set_location (const char *pos)
{
  if (!pos)
    return FALSE;
...
int
VMDApp::stage_set_numpanels (int num)
{
  if (!stage->panels (num))
    return FALSE;
...
int
VMDApp::axes_set_location (const char *pos)
{
  if (!pos)
    return FALSE;
...
int
VMDApp::light_on (int n, int onoff)
{
...
int
VMDApp::light_highlight (int n, int onoff)
{
  if (n < 0 || n >= DISP_LIGHTS)
    return FALSE;
...
int
VMDApp::light_rotate (int n, float amt, char axis)
{
  if (n < 0 || n >= DISP_LIGHTS)
    return FALSE;
...
int
VMDApp::light_move (int n, const float *newpos)
{
  if (n < 0 || n >= DISP_LIGHTS)
    return FALSE;
...
int
VMDApp::depthcue_set_mode (const char *mode)
{
  if (!display->set_cue_mode (mode))
    return FALSE;
...
int
VMDApp::depthcue_set_start (float val)
{
  if (!display->set_cue_start (val))
    return FALSE;
...
int
VMDApp::depthcue_set_end (float val)
{
  if (!display->set_cue_end (val))
    return FALSE;
...
int
VMDApp::depthcue_set_density (float val)
{
  if (!display->set_cue_density (val))
    return FALSE;
...
int
VMDApp::display_set_shadows (int onoff)
{
  if (!display->set_shadow_mode (onoff))
    return FALSE;
...
int
VMDApp::display_set_ao (int onoff)
{
  if (!display->set_ao_mode (onoff))
    return FALSE;
...
int
VMDApp::display_set_ao_ambient (float val)
{
  if (!display->set_ao_ambient (val))
    return FALSE;
...
int
VMDApp::display_set_ao_direct (float val)
{
  if (!display->set_ao_direct (val))
    return FALSE;
...
void
VMDApp::deactivate_uitext_stdin ()
{
  if (uiText)
    uiText->Off ();
}
...
int
VMDApp::label_add (const char *category, int n, const int *molids,
    const int *atomids, const int *cells, float k, int toggle)
{
  if (!category || !molids || !atomids)
    return -1;
  ...
  if (rc >= 0)
  {
    if (!strcmp (category, "Springs"))
      commandQueue->runcommand (
          new CmdLabelAddspring (molids[0], atomids[0], atomids[1], k));
    else
      commandQueue->runcommand (
          new CmdLabelAdd (category, n, (int *) molids, (int *) atomids));
  }
  return rc;
}

int
VMDApp::label_show (const char *category, int n, int onoff)
{
  if (!category)
    return FALSE;
...
float
VMDApp::label_get_textsize () const
{
  return geometryList->getTextSize ();
}

int
VMDApp::label_set_textsize (float newsize)
{
...
int
VMDApp::label_set_textoffset (const char *nm, int n, float x, float y)
{
  float delta[2] =
    { x, y };
...
int
VMDApp::label_set_textformat (const char *nm, int n, const char *format)
{
...
int
VMDApp::label_delete (const char *category, int n)
{
  if (!category)
    return FALSE;
...
int
VMDApp::tool_create (const char *type, int argc, const char **argv)
{
  if (!uivr)
    return FALSE;
  if (!uivr->add_tool_with_USL (type, argc, argv))
    return FALSE;
...
int
VMDApp::tool_change_type (int toolnum, const char *type)
{
  if (!uivr)
    return FALSE;
  if (!uivr->change_type (toolnum, type))
    return FALSE;
...
int
VMDApp::tool_delete (int toolnum)
{
  if (!uivr)
    return FALSE;
  if (!uivr->remove_tool (toolnum))
    return FALSE;
...
int
VMDApp::tool_set_position_scale (int toolnum, float newval)
{
  if (!uivr)
    return FALSE;
...
int
VMDApp::tool_set_force_scale (int toolnum, float newval)
{
  if (!uivr)
    return FALSE;
...
int
VMDApp::tool_set_spring_scale (int toolnum, float newval)
{
  if (!uivr)
    return FALSE;
...
const char *
VMDApp::material_add (const char *name, const char *copy)
{
...
int
VMDApp::material_delete (const char *name)
{
...
int
VMDApp::material_rename (const char *prevname, const char *newname)
{
  char * oldname = stringdup (prevname);
  int ind = materialList->material_index (oldname);
  if (ind < 0)
  {
    msgErr << "material rename: '" << oldname << "' does not exist." << sendmsg;
    delete[] oldname;
    return FALSE;
  }
  ...
  {
    msgErr << "material rename: '" << newname << "' already exists." << sendmsg;
    delete[] oldname;
    return FALSE;
  }
...
int
VMDApp::material_change (const char *name, int property, float val)
{
  ...
  {
    case MAT_AMBIENT:
      materialList->set_ambient (ind, val);
      break;
    case MAT_SPECULAR:
      materialList->set_specular (ind, val);
      break;
    case MAT_DIFFUSE:
      materialList->set_diffuse (ind, val);
      break;
    case MAT_SHININESS:
      materialList->set_shininess (ind, val);
      break;
    case MAT_OPACITY:
      materialList->set_opacity (ind, val);
      break;
    case MAT_OUTLINE:
      materialList->set_outline (ind, val);
      break;
    case MAT_OUTLINEWIDTH:
      materialList->set_outlinewidth (ind, val);
      break;
  }
...
int
VMDApp::material_restore_default (int ind)
{
...
int
VMDApp::mouse_set_mode (int mm, int ms)
{
  ...
  {
    case Mouse::PICK:
      pickModeList->set_pick_mode (PickModeList::PICK);
      break;
    case Mouse::QUERY:
      pickModeList->set_pick_mode (PickModeList::QUERY);
      break;
    case Mouse::CENTER:
      pickModeList->set_pick_mode (PickModeList::CENTER);
      break;
    case Mouse::LABELATOM:
      pickModeList->set_pick_mode (PickModeList::LABELATOM);
      break;
    case Mouse::LABELBOND:
      pickModeList->set_pick_mode (PickModeList::LABELBOND);
      break;
    case Mouse::LABELANGLE:
      pickModeList->set_pick_mode (PickModeList::LABELANGLE);
      break;
    case Mouse::LABELDIHEDRAL:
      pickModeList->set_pick_mode (PickModeList::LABELDIHEDRAL);
      break;
    case Mouse::MOVEATOM:
      pickModeList->set_pick_mode (PickModeList::MOVEATOM);
      break;
    case Mouse::MOVERES:
      pickModeList->set_pick_mode (PickModeList::MOVERES);
      break;
    case Mouse::MOVEFRAG:
      pickModeList->set_pick_mode (PickModeList::MOVEFRAG);
      break;
    case Mouse::MOVEMOL:
      pickModeList->set_pick_mode (PickModeList::MOVEMOL);
      break;
    case Mouse::FORCEATOM:
      pickModeList->set_pick_mode (PickModeList::FORCEATOM);
      break;
    case Mouse::FORCERES:
      pickModeList->set_pick_mode (PickModeList::FORCERES);
      break;
    case Mouse::FORCEFRAG:
      pickModeList->set_pick_mode (PickModeList::FORCEFRAG);
      break;
    case Mouse::MOVEREP:
      pickModeList->set_pick_mode (PickModeList::MOVEREP);
      break;
    case Mouse::ADDBOND:
      pickModeList->set_pick_mode (PickModeList::ADDBOND);
      break;
    default:
      break;
  }
...
int
VMDApp::spaceball_set_mode (int mm)
{
...
int
VMDApp::spaceball_set_sensitivity (float s)
{
...
int
VMDApp::spaceball_set_null_region (int nr)
{
...
void
VMDApp::spaceball_get_tracker_status (float &tx, float &ty, float &tz,
...
int
VMDApp::textinterp_change (const char *name)
{

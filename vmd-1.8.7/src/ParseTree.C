 ...
 *  $Revision: 1.127 $		$Date: 2009/07/07 19:13:27 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
static void same_int(symbol_data *tmp, symbol_data *tmp2, int num, 
  ...
  int i;
  for (i=0; i<num; i++) if (subselect[i]) break;
  if (i==num) {
    // subselection is empty, so set all flags to zero.
    memset(flgs, 0, num*sizeof(int));
    return;
  }
  int_min = int_max = tmp2->ival[i];
  while (++i < num) {
    if (!subselect[i]) continue;
...
static void same_double(symbol_data *tmp, symbol_data *tmp2, int num, 
  ...
  hash_t hash;
  int i, n=0;
  for (i=0; i<num; i++) n += subselect[i];
  char *doublestring = new char[25*n];  // XXX doubles can't be longer than 25 
  char *istring = doublestring;         // characters, can they?
  hash_init(&hash, n);
  for (i=0; i<num; i++) 
    if (subselect[i]) {
...
void ParseTree::eval_same(atomparser_node *node, int num, int *flgs) {
   ...
   for (i=0; i<num; subselect[i++]=1); // set subselect array to 1
...
void ParseTree::eval_within_bonds(atomparser_node *node, int num, int *flgs) {
  ...
  // copy others to flgs
  memcpy(flgs, others, num*sizeof(int));

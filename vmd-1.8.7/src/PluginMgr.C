 ...
 *      $Revision: 1.34 $       $Date: 2009/07/07 02:40:05 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int PluginMgr::load_sharedlibrary_plugins(const char *fullpath) {
  ...
  if (!handle) {
    msgErr << "Unable to open dynamic library '" << fullpath << "'." << sendmsg;
    msgErr << vmddlerror() << sendmsg; 
    return -1;
  }
  ...
  if (ifunc && ((initfunc)(ifunc))()) {
    msgErr << "vmdplugin_init() for " << fullpath 

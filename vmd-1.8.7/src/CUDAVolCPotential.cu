 ...
 *      $Revision: 1.37 $      $Date: 2009/07/22 20:17:34 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int copyatomstoconstbuf(const float *atoms, float *atompre, 
...
int vmd_cuda_vol_cpotential(long int natoms, float* atoms, float* grideners,
  ...
  globaltimer = vmd_timer_create();
  vmd_timer_start(globaltimer);

  /* spawn child threads to do the work */
  vmd_tasktile_t tile;
  tile.start=0;
  tile.end=numplane;
  rc = vmd_threadlaunch(numprocs, &parms, cudaenergythread, &tile);

  // Measure GFLOPS
  vmd_timer_stop(globaltimer);
  totalruntime = vmd_timer_time(globaltimer);
  vmd_timer_destroy(globaltimer);
...
#define DMABUFPADSIZE (32 * 1024)
  ...
  vmd_tasktile_t tile;
  while (vmd_threadlaunch_next_tile(voidparms, 1, &tile) != VMD_SCHED_DONE) {
    int k;
    for (k=tile.start; k<tile.end; k++) {
      ...
      lasttime = vmd_timer_timenow(timer);
      ...
      totaltime = vmd_timer_timenow(timer);
  ...
  vmd_timer_destroy(timer); // free timer
  msg_timer_destroy(msgt); // free timer

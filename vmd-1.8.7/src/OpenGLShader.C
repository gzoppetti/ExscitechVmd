 ...
 *	$Revision: 1.22 $	$Date: 2009/04/29 15:43:13 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int OpenGLShader::ReadShaderSource(const char *filename, GLubyte **vertexShader, GLubyte **fragmentShader) {
  ...
  FILE *vsfp, *fsfp;
  long vsize, fsize, count;

  vsfilename = (char *) malloc(strlen(filename) + strlen(".vert") + 1);
  strcpy(vsfilename, filename); 
  strcat(vsfilename, ".vert"); 
  vsfp = fopen(vsfilename, "r");

  fsfilename = (char *) malloc(strlen(filename) + strlen(".frag") + 1);
  ...
  *vertexShader = (GLubyte *) malloc(vsize + 1);
  memset(*vertexShader, 0, vsize + 1);
  count=fread(*vertexShader, 1, vsize, vsfp);

  // find size and load fragment shader
  fseek(fsfp, 0, SEEK_END);
  fsize = ftell(fsfp); 
  fseek(fsfp, 0, SEEK_SET);
  *fragmentShader = (GLubyte *) malloc(fsize + 1);
  memset(*fragmentShader, 0, fsize + 1);
  count=fread(*fragmentShader, 1, fsize, fsfp);

 ...
 *      $Revision: 1.103 $       $Date: 2009/04/29 15:43:14 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
void PSDisplayDevice::render(const VMDDisplayList *display_list) {
   ...
   while ((tok = display_list->next(&cmditer, cmd_ptr)) != DLASTCOMMAND) {
      ...
      switch (tok) {
         case DDATABLOCK:
            #ifdef VMDCAVE
               dataBlock = (float *) cmd_ptr;
            #else
               dataBlock = ((DispCmdDataBlock *) cmd_ptr)->data;
            #endif
            break;
         ...
         case DSPHERE_I:
         {
            ...
            i = ((DispCmdSphereIndex *) cmd_ptr)->pos;
            (transMat.top()).multpoint3d(&dataBlock[i], c);
            r = scale_radius(((DispCmdSphereIndex *) cmd_ptr)->rad);

 ...
 *      $Revision: 1.145 $       $Date: 2009/07/28 22:11:54 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
// XXX Only checks leaf node menu names, not full pathnames currently
//
...
// XXX Only checks the leaf node menu names, not full pathnames currently
//
static void set_chooser_from_string(const char *namestr,
...
: VMDFltkMenu("graphics", "Graphical Representations", vmdapp) {
  ...
  { //Fl_Window* o = new Fl_Window(grwwidth, 541);
    ...
    { Fl_Choice* o = materialchooser = new Fl_Choice(180, 320, 120, 25, "Material");
      VMDFLTKTOOLTIP(o, "Select a material for shading geometry")
      o->color(VMDMENU_CHOOSER_BG);
      o->selection_color(VMDMENU_CHOOSER_SEL);
      o->box(FL_THIN_UP_BOX);
      o->align(FL_ALIGN_TOP);
    ...
    cbdata = new isosurface_cbdata;
    cbdata->self = this;
    cbdata->isorep = new GraphicsFltkRepIsosurface(isosurfacerepcontrol_cb, cbdata);
    repcontrols.add_name("Isosurface", cbdata->isorep);

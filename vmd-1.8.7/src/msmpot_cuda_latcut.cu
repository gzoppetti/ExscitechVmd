...
int Msmpot_cuda_setup_latcut(MsmpotCuda *mc) {
  ...
  if (h != msm->hy || h != msm->hz) return ERROR(MSMPOT_ERROR_AVAIL);

  if (nlevels > MAXLEVELS) return ERROR(MSMPOT_ERROR_AVAIL);
  mc->lk_nlevels = nlevels;
  nrad = (int) ceilf(2*a/h) - 1;
  srad = (int) ceilf((nrad + 1) / 4.f);
  if (srad > 3) return ERROR(MSMPOT_ERROR_AVAIL);
  mc->lk_srad = srad;
  ...
  for (i = 0;  i < nlevels;  i++) {
    printf("ni=%d  nj=%d  nk=%d\n",
        mc->qh[i]->ni, mc->qh[i]->nj, mc->qh[i]->kb - mc->qh[i]->ka + 1);
...
#define MAX_GRID_DIM  65536u
  ...
  if (bx > MAX_GRID_DIM || by > MAX_GRID_DIM) {
    ...
    return ERROR(MSMPOT_ERROR_AVAIL);
  }

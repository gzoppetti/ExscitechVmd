 ...
 *	$Revision: 1.144 $	$Date: 2009/04/29 15:43:01 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int
DrawMolItem::draw_volume_get_colorid (void)
{
  int colorid = 0;
  int catindex;
...
void
DrawMolItem::draw_volume_box_solid (const VolumetricData * v)
{
...
void
DrawMolItem::draw_volume_box_lines (const VolumetricData * v)
{
  float start[3], end[3];
...
void
DrawMolItem::draw_volume_isosurface_points (const VolumetricData * v,
    float isovalue, int stepsize, int thickness)
{
  ...
  if (pointcount > 0)
  {
    cmdPointArray.putdata ((float *) &centers[0], (float *) &colors[0],
        (float) thickness, pointcount, cmdList);
  }
}
...
// Exscitech - generate your volume trimesh!
void
DrawMolItem::generate_volume_isosurface_trimesh (const VolumetricData * v,
...
static void
prepare_texture_coordinates (const VolumetricData *v, float loc, int axis,
    float *sliceNormal, float *sliceTextureCoords, float *sliceVertexes)
{
...
void
DrawMolItem::updateVolumeTexture ()
{
  float vmin, vmax;
...
void
DrawMolItem::draw_volslice (int volid, float slice, int axis, int texmode)
{
  const VolumetricData *v = mol->get_volume_data (volid);
  float sliceNormal[3];
  ...
  float tscale[3] =
    { float (v->xsize), float (v->ysize), float (v->zsize) };
  ...
  cmdVolSlice.putdata (texmode, sliceNormal, sliceVertexes, sliceTextureCoords,
      cmdList);
}

void
DrawMolItem::draw_isosurface (int volid, float isovalue, int drawbox, int style,
    int stepsize, int thickness)
{
...
int
DrawMolItem::calcseeds_gradient_magnitude (const VolumetricData * v,
    ...
    for (z = 0; z < v->zsize && seedcount < maxseedcount; z += stepsize)
    {
      for (y = 0; y < v->ysize; y += stepsize)
      {
        for (x = 0; x < v->xsize; x += stepsize)
        {
          ...
          if ((gradmag2 <= seedmax2) && (gradmag2 >= seedmin2))
          {
            pos[0] = vorigin[0] + x * xax[0] + y * yax[0] + z * zax[0];
            pos[1] = vorigin[1] + x * xax[1] + y * yax[1] + z * zax[1];
            pos[2] = vorigin[2] + x * xax[2] + y * yax[2] + z * zax[2];

 ...
 *      $Revision: 1.160 $      $Date: 2009/08/01 06:38:44 $
 *
 ***************************************************************************/
...
void VolMapCreateILS::set_probe(int num_atoms, int num_conf,
                                const float *pcoords,
                                const float *vdwrmin,
                                const float *vdweps,
                                const float *charge) {
  if (num_atoms<=0) return;
  if (num_atoms==1 && num_conf>0) num_conf = 0;

  conformer_freq  = num_conf;
  num_probe_atoms = num_atoms;
  probe_coords = new float[3*num_atoms];
  probe_vdw    = new float[2*num_atoms];
  probe_charge = new float[num_atoms];
  ...
  if (charge) {
    memcpy(probe_charge, charge, num_atoms*sizeof(float));
  } else {
    memset(probe_charge, 0, num_atoms*sizeof(float));
  }
...
static void dodecahedron(float *faces, float *vertices, int C2symm) {
  ...
  const float dihedral = 180.f-RADTODEG(acos(-1.f/sqrtf(5.f)));
  ...
  rot.identity();
  rot.rotate_axis(cross, DEGTORAD(-phi));
...
float VolMapCreateILS::dimple_depth(float phi) {
  ...
  for (i=0; i<num_probe_atoms; i++) {
    ...
    float a = d*sinf(DEGTORAD(phi));
    float m = a/cosf(DEGTORAD(phi));
    if (phi == 90.f) m = a;
    ...
    float depth = d + c - d*cosf(DEGTORAD(phi)) - b;
  ...
  for (i=0; i<numorient; i++) {
    ...
    for (j=0; j<numrot; j++) {
      ...
      if (i>=numorient/2) {
        ...
        float z[3];
        vec_zero(z); z[2]=1.f;
        rot.rotate_axis(z, DEGTORAD(180.f));
      }

      // Rotate around dir
      rot.rotate_axis(dir, DEGTORAD(psi));
...
int VolMapCreateILS::gen_conf(float *(&conform), int freq,
  ...
  case 4:
    ...
    anglespacing = 180.f-RADTODEG(acos(-1.f/sqrtf(5.f)));
      break;
  case 5:
  ...
  for (i=0; i<numorient; i++) {
    ...
    for (j=0; j<numrot; j++) {
      ...
      // Rotate around dir
      m.rotate_axis(dir, DEGTORAD(psi));

      // Rotate around X
      m.rot(theta, 'x');
      
      // Tilt X into dir
      m.rotate_axis(z, DEGTORAD(phi));
...
void VolMapCreateILS::initialize_probe() {
  ...
  if (probe_axisorder2>1) {
    ...
    rot.identity();
    rot.rotate_axis(probe_symmaxis1, DEGTORAD(phi));
...
int VolMapCreateILS::compute() {
  ...
  vmd_timerhandle alltimer = vmd_timer_create();
  vmd_timer_start(alltimer);
  ...
  for (frame=first; frame<=last; frame++) { 
    ...
    msgInfo << "Total frame time = " << vmd_timer_timenow(timer) << " s" << sendmsg;
  ...
  double allframetime = vmd_timer_timenow(alltimer);
  ...
  msgInfo << "Downsampling              = "
          << vmd_timer_timenow(alltimer)-allframetime << " s"
          << sendmsg << sendmsg;
  msgInfo << "#################################################"
          << sendmsg << sendmsg;

  vmd_timer_destroy(timer);
  vmd_timer_destroy(alltimer);
...
#ifdef TIMING
  msgInfo << "Coord setup: " << vmd_timer_timenow(timer) << " s" << sendmsg;
#endif
  vmd_timer_destroy(timer);
...
int VolMapCreateILS::compute_frame(int frame, float *voldata) { 
...
#ifdef TIMING
    msgInfo << "Masking:     " << vmd_timer_timenow(timer) << " s" << sendmsg;
#endif
  ...
  int *vdwtypes = NULL;
  int numcoords = get_atom_coordinates(frame, alignment,
                                       vdwtypes, coords);
...
int ComputeOccupancyMap_calculate_slab(ComputeOccupancyMap *p) {
  ...
  if (1 == p->num_probes) {
    ...
    sprintf(report, "ComputeOccupancyMap: find_energy_exclusions()          "
        "%f s  -> %d exclusions\n", vmd_timer_timenow(timer), numexcl);
    msgInfo << report << sendmsg;
#endif
#endif

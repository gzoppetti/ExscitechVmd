 ...
 *      $Revision: 1.30 $       $Date: 2009/04/29 15:43:38 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
static int vmdinfo_tcl(ClientData, Tcl_Interp *interp,
  ...
  if (argc == 2) {
    ...
    if (!strcmp(argv[1], "numcpus")) {
      ...
      int numcpus = vmd_thread_numprocessors();
#else
      int numcpus = 1;
#endif
      Tcl_Obj *tcl_result = Tcl_NewListObj(0, NULL);
...
#if defined(VMDTHREADS)
      cpuaffinitylist = vmd_cpu_affinitylist(&numcpus);
#endif
...
#if defined(VMDTK) && !defined(_MSC_VER) && !defined(MINGW)

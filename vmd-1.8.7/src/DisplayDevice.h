 ...
 *	$Revision: 1.128 $	$Date: 2009/05/17 06:37:37 $
 *
 ***************************************************************************/
...
public:
  ...
  enum CursorCodes
  {
    NORMAL_CURSOR, TRANS_CURSOR, SCALE_CURSOR, PICK_CURSOR, WAIT_CURSOR
  };

  /// display state has changed, requiring a redraw
  int
  needRedraw (void) const
  {
    return _needRedraw;
  }
  ...
  // Exscitech: Make OpenGL context current
  //              To avoid problems when multiple contexts are used
  virtual void
  makeContextCurrent ()
  {
  }
...
protected:
  ...
  enum CueMode
  {
    CUE_LINEAR, CUE_EXP, CUE_EXP2, NUM_CUE_MODES
  };
...
public:
  ...
  enum Projection
  {
    PERSPECTIVE, ORTHOGRAPHIC, NUM_PROJECTIONS
  };
  Projection
  projection () const
  {
  ...
  // Exscitech:
  void
  getClipPlanes (float& leftClip, float& rightClip, float& bottomClip,
      float& topClip) const
  {
    leftClip = cpLeft;
    rightClip = cpRight;
...
public:
  ...
  virtual int
  do_define_light (int, float *, float *)
  {
    return TRUE;
  }
  virtual int
  do_activate_light (int, int)
  {
    return TRUE;
  }
...
protected:
  ...
  virtual void
  do_reposition_window (int xpos, int ypos)
  {
  }
  ...
  /// the given array.
  void
  find_pbc_images (const VMDDisplayList *, ResizeArray<Matrix4> &);
  ...
  void
  find_pbc_cells (const VMDDisplayList *, ResizeArray<int> &);
...
public:
   ...
   */
  virtual int
  read_event (long &, long &);

  // get the current state of the device's pointer (i.e. cursor if it has one)
  virtual int
  x (void); /// absolute position of cursor from lower-left corner
  virtual int
  y (void); /// absolute position of cursor from lower-left corner

  /// Mouse and keyboard shift state, joystick/sball aux key/button state  
  enum
  {
    SHIFT = 1, CONTROL = 2, ALT = 4, AUX = 8
  };

  virtual int
  shift_state (void); /// return the shift state (ORed enums)
   ...
   */
  virtual int
  spaceball (int *, int *, int *, int *, int *, int *, int *)
  {
    return 0;
  }
...
public:
  ...
  float
  clip_width (void) const
  {
    return (farClip - nearClip);
  }
  float
  addto_near_clip (float ac)
  {
    return set_near_clip (nearClip + ac);
  }
  float
  addto_far_clip (float ac)
  {
    return set_far_clip (farClip + ac);
  }
  float
  set_near_clip (float nc)
  {
  ...
  get_cue_mode () const
  {
    return cueModeNames[cueMode];
  }
  int
  num_cue_modes () const
  {
    return NUM_CUE_MODES;
  }
  const char *
  cue_mode_name (int i) const
  {
    if (i < 0 || i >= NUM_CUE_MODES)
      return NULL;
    return cueModeNames[i];
  }
  int
  set_cue_mode (const char *mode)
  {
  ...
  float
  get_cue_density () const
  {
    return cueDensity;
  }
  int
  set_cue_density (float d)
  {
  ...
  culling_off (void);
  int
  culling_available (void)
  {
    return cullingAvailable;
  }
  int
  culling_enabled (void)
  {
  ...
  float
  get_ao_ambient () const
  {
    return aoAmbient;
  }
  int
  set_ao_ambient (float a)
  {
    aoAmbient = a;
    _needRedraw = 1;
    return TRUE;
  ...
  float
  distance_to_screen (void)
  {
    return zDist;
  }
  void
  distance_to_screen (float zd)
  {
    zDist = zd;
    calc_frustum ();
    _needRedraw = 1;
  ...
  void
  printProjectionValues ()
  {
    fprintf (
        stderr,
        "Aspect: %f vSize: %f zDist: %f Left: %f Right: %f Top: %f Bottom: %f eyeZ: %f nearClip: %f\n",
        Aspect, vSize, zDist, cpLeft, cpRight, cpUp, cpDown, eyePos[2],
        nearClip);
  }

  void
  setClipPlanes (float rightPlane, float leftPlane, float topPlane,
      float bottomPlane, float nearPlane, float farPlane, float verticalSize)
  {
    cpRight = rightPlane;
    cpLeft = leftPlane;
    cpUp = topPlane;
  ...
  void
  setClipPlanes (float rightPlane, float aspectRatio, float nearPlane,
      float farPlane)
  {
    cpRight = rightPlane;
    cpLeft = -cpRight;
  ...
  void
  setClipPlanes (float rightPlane, float aspectRatio)
  {
    Aspect = aspectRatio;
    cpRight = rightPlane;
    cpLeft = -cpRight;
...
/// a) specify aspect ratio
  void
  set_screen_pos (float vsize, float zdist, float asp)
  {
    Aspect = asp;
    vSize = vsize;
    zDist = zdist;
...
/// b) have device provide aspect ratio
  void
  set_screen_pos (float vs, float zd)
  {
    set_screen_pos (vs, zd, aspect ());
  }

/// c) just specify aspect ratio
  void
  set_screen_pos (float asp)
  {
    set_screen_pos (vSize, zDist, asp);
  }
...
/// Reposition the window
  void
  reposition_window (int xpos, int ypos)
  {
...
/// set default eye position, orientation information
  int
  set_eye_defaults (void);

/// set eye position
  int
  set_eye_pos (const float * pos)
  {
    if (!pos)
      return FALSE;
...
//Exscitech: Added const correctness.
/// set eye direction
  int
  set_eye_dir (const float * dir)
  {
    if (!dir)
      return FALSE;
...
/// find the direction to the right eye position
  void
  right_eye_dir (float& x, float& y, float& z) const
  {
    x = eyeSepDir[0];
    y = eyeSepDir[1];
    z = eyeSepDir[2];
...
/// get/change the viewing matrix
  int
  num_projections () const
  {
    return NUM_PROJECTIONS;
  }
  const char *
  projection_name (int i) const
  {
    if (i < 0 || i >= NUM_PROJECTIONS)
      return NULL;
    return projNames[i];
  }
  const char *
  ...
  {
    return projNames[my_projection];
  }
  int
  set_projection (const char *proj)
  {
    if (!proj)
      return FALSE;
...
/// convert 2D relative screen coords into absolute coords
  void
  abs_screen_pos (float &x, float &y)
  {
...
// which goes with the second 2D point at pos B.  Result returned in 3rd arg.
  virtual void
  find_3D_from_2D (const float *, const float *, float *)
  {
  }
...
//
  virtual void
  loadmatrix (const Matrix4 &); ///< replace transformation matrix
  virtual void
  multmatrix (const Matrix4 &); ///< multiply transformation matrix

//
// set the background colors
//
  void
  set_background_mode (int newmode)
  {
    backgroundmode = newmode;
  } ///< set display background type
  int
  background_mode (void)
  {
    return backgroundmode;
  } ///< return active background mode
  virtual void
  set_background (const float *)
  {
  } ///< set main background color
  virtual void
  set_backgradient (const float *, const float *)
  {
  } ///< set gradient colors
...
//
  virtual int
  prepare3D (int = TRUE); ///< ready to draw 3D
  virtual int
  prepareOpaque (void)
  {
    return 1;
  ...
  virtual void
  clear (void); ///< erase the device
  virtual void
  left (void); ///< ready to draw left eye
  virtual void
  right (void); ///< ready to draw right eye
  virtual void
  normal (void); ///< ready to draw non-stereo
  virtual void
  update (int = TRUE); ///< finish up after drawing
  virtual void
  reshape (void); ///< refresh device after change
...
// The window_size argument tells pick how close the picked item must be.
  virtual int
  pick (int, const float *, const VMDDisplayList *, float &, int *,
      float window_size);

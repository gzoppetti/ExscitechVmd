 ...
 *      $Revision: 1.107 $       $Date: 2009/08/01 04:13:29 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
public:
  ...
  void regen_excitationlist() {
    ...
    if (m != NULL && m->qm_data != NULL && 
        m->qm_data->has_wavef_type(wftype)) {
      ...
      for (i=0; i<=maxexcitations; i++) {
        if (m->qm_data->has_wavef_signa(wftype, GUI_WAVEF_SPIN_ALPHA, i) ||
            m->qm_data->has_wavef_signa(wftype, GUI_WAVEF_SPIN_BETA, i)) {

 ...
 *      $Revision: 1.4 $      $Date: 2009/04/29 15:42:51 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
int stream_bench(int N, double *time, double *mbsec) {
  ...
  if ((da != NULL) && (db != NULL) && (dc != NULL)) {
    ...
    time[0] = vmd_timer_time(timer);
    mbsec[0] = mbsz / time[0];

    vmd_timer_start(timer);
    dstream_scale(da, db, 2.0, N, &mbsz);
    vmd_timer_stop(timer);
    time[1] = vmd_timer_time(timer);
    mbsec[1] = mbsz / time[1];

    vmd_timer_start(timer);
    dstream_add(da, db, dc, N, &mbsz);
    vmd_timer_stop(timer);
    time[2] = vmd_timer_time(timer);
    mbsec[2] = mbsz / time[2];

    vmd_timer_start(timer);
    dstream_triad(da, db, dc, 2.0, N, &mbsz);
    vmd_timer_stop(timer);
    time[3] = vmd_timer_time(timer);
    mbsec[3] = mbsz / time[3];
  } else {
    rc = -1;
  }
  ...
  if ((fa != NULL) && (fb != NULL) && (fc != NULL)) {
    ...
    vmd_timer_stop(timer);
    time[4] = vmd_timer_time(timer);
    mbsec[4] = mbsz / time[4];

    vmd_timer_start(timer);
    fstream_scale(fa, fb, 2.0, N, &mbsz);
    vmd_timer_stop(timer);
    time[5] = vmd_timer_time(timer);
    mbsec[5] = mbsz / time[5];

    vmd_timer_start(timer);
    fstream_add(fa, fb, fc, N, &mbsz);
    vmd_timer_stop(timer);
    time[6] = vmd_timer_time(timer);
    mbsec[6] = mbsz / time[6];

    vmd_timer_start(timer);
    fstream_triad(fa, fb, fc, 2.0, N, &mbsz);
    vmd_timer_stop(timer);
    time[7] = vmd_timer_time(timer);
    mbsec[7] = mbsz / time[7];
  } else {
    rc = -1;
  }

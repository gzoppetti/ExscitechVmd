 ...
 *	$Revision: 1.159 $	$Date: 2009/06/16 15:15:11 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int access_tcl_atomsel(ClientData my_data, Tcl_Interp *interp,
  ...
  case 4: { // get
    ...
    if (num_mapping == 1) {
      ...
      if (elem->is_a == SymbolTableElement::SINGLEWORD) {
        ...
        elem->keyword_single(&context, num_atoms, flgs);
          ...
          case (SymbolTableElement::IS_STRING):
            {
              ...
              elem->keyword_string(&context, num_atoms, tmp, atomSel->on);
              for (int j=0; j<num_atoms; j++) {
                if (atomSel->on[j])
          ...
          case (SymbolTableElement::IS_INT):
            {
              ...
              elem->keyword_int(&context, num_atoms, tmp, atomSel->on);
              for (int j=0; j<num_atoms; j++) {
                if (atomSel->on[j])
                  Tcl_ListObjAppendElement(interp, result,
                    Tcl_NewIntObj(tmp[j]));
          ...
          case (SymbolTableElement::IS_FLOAT):
            {
              ...
              elem->keyword_double(&context, num_atoms, tmp, atomSel->on);
              for (int j=0; j<num_atoms; j++) {
                if (atomSel->on[j])
                  Tcl_ListObjAppendElement(interp, result,
  ...
  case 23: {   // writeXXX <name>
    ...
    if (!app->molecule_valid_id(molid)) {
      sprintf(interp->result, "atomsel: writeXXX: was molecule %d deleted?",
          molid);
      return TCL_ERROR;
    }

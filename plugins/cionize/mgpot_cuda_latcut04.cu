...
__global__ static void latcut_kernel(
    uint nsubcubes,  /* total number of subcubes */
    ...
    )
{
  ...
  uint block_index = gridDim.x * blockIdx.y + blockIdx.x;
...
#define MAX_GRID_DIM  65536u
  ...
  if (bx * by < (uint)(mg->block_total)) bx++;

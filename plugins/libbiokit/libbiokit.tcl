...
#         puts "getQPerResidue: inside IF.  filePrefix: $filePrefix, tempdir: $tempDir"
         ...
         if {$type == "dna" || $type == "rna" } {
            ...
            puts "libbiokit.getQPerResidue.  bad type: $type"
         }
      ...
      for {set i 0} {$i < [llength $sequenceIDs]} {incr i} {
         ...
         if {$index != -1} {
            ...
            if {$residueIndex != [llength $residueScores]} {
               error "getQPerResidue: An unknown error occurred while processing [::SeqData::getName $sequenceID]: $residueIndex, [llength $residueScores]"
            }
    ...
    proc writeStructureFiles {sequenceIDs} {
        ...
        for {set i 0} {$i < [llength $newSequenceIDs]} {incr i} {
            ...
            if {$alignmentLength == -1} {
                ...
                error "Writing Structure Files: The operation could not be performed because the sequences were not aligned."
            }   
        }

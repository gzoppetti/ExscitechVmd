...
// printf ("tcl_libbiokit.cpp.seq_new. seqData is '%s'\n", sequenceData);
...
//        printf("precolors in seq_set: "); ((ShortIntList*)(colorings->get(sequenceID)))->printList();
        ...
        for(i=0; i<size; i++) 
        {
           if(sequenceData[(2*i)] == '-') 
           {
              ...
              {
                 colors->add(((ShortIntList*)(colorings->get(sequenceID)))->get(origIndex));
                 origIndex++;
              }
           } 
        ...
        sequence = new AlignedSequence(alphabet);
        addSequenceData(sequenceData, sequence, colors);
...
const char* seq_residue_at_position(int argc, const char* argv[])
{
    if (argc == 3)
    {
        ...
        if (positionIndex == -1)
        {
            printf("[seq resAt] Invalid position: %s\n", argv[2]);
            return "";
        }
...
const char* seq_get_color(int argc, const char* argv[])
{
    if (argc == 4)
    {
        ...
        int positionIndex = parsePositiveInteger(argv[3], sequence->getNumberPositions()-1);
        if (positionIndex == -1)
        {
            printf("[seq get color] Invalid position: %s\n", argv[3]);
            return "";
        }
...
const char* seq_set_color(int argc, const char* argv[])
{
    if (argc == 5 || argc == 6)
    {
        ...
        if (sequence == NULL)
        {
            printf("[seq set color] Invalid sequence id: %s\n", argv[2]);
            return "";
        }
        int startPositionIndex = parsePositiveInteger(argv[3], sequence->getNumberPositions()-1);
        if (startPositionIndex == -1)
        {
            printf("[seq set color] Invalid position: %s\n", argv[3]);
            return "";
        }
        int colorArg = 4;
        ...
        {
            colorArg = 5;
            endPositionIndex = parsePositiveInteger(argv[4], sequence->getNumberPositions()-1);
            ...
            {
                printf("[seq set color] Invalid range: %s - %s\n", argv[3], argv[4]);
                return "";
            }
        }
        ...
        if (color == -1)
        {
            printf("[seq set color] Invalid color: %s\n", argv[colorArg]);
            return "";
        }
...
/* ----------------------------------------------------------------------- */
void addSequenceData(const char *sequenceData, AlignedSequence* sequence, ShortIntList* colors)
{
   int i;
    ...
    if (strstr(sequenceData, " ") != NULL)
    {
        ...
        if (sequenceData[0] != ' ') sequence->add(sequenceData[0]);
        ...
        while (sequenceData != NULL)
        {
            ...
            if (sequenceData[0] != '\0') sequence->add(sequenceData[0]);

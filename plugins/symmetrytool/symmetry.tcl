...
package provide symmetrytool 1.2
...
proc ::Symmetry::create_gui {args} {
   ...
   menubutton $f.inputmol -relief raised -bd 2 -direction flush \
       -textvariable ::Symmetry::origmolname \
...
proc ::Symmetry::draw_symaxis {sel pivot axis args} {
   set color  [getargs $args "-color" orange3]
   set label  [getargs $args "-label" {}]
   set radius [getargs $args "-radius" 0.05]
...
proc ::Symmetry::draw_plane {pivot normal args} {
   set radius   [getargs $args "-radius" 1.0]
   set axis     [getargs $args "-axis" {}]
   set nvplanes [getargs $args "-nvplanes" 1]
   set color    [getargs $args "-color" pink]
   set n [getargs $args "-n" 50]
...
proc ::Symmetry::draw_scaled_arrow {mol start end args} {
   set res    [expr {   int([getargs $args "resolution" 6])}]
   #set rad    [expr {double([getargs $args "radius"  1.0])}]
   #set aspect [expr {double([getargs $args "aspect" 5.0])}]
...
proc ::Symmetry::vmd_draw_disc {mol center normal discrad args} {
   #set ringradius [expr {double([getargs $args "ringradius"  0.0])}]

...
proc ::SeqData::getLen { sequenceID } {
    variable seqs
    if {[info exists seqs($sequenceID)]} {
...
#        puts "seqdata.getLineage. seqNum: $seqNum, showHid: $showHidden, includeRanks: $includeRanks, incSelf: $includeSelf, lineage: '$lineage', taxNode: '$seqs($seqNum,taxonomyNode)'" 
...
proc ::SeqData::getTemperatureClass {sequenceID} {
    ...
    if {[info exists seqs($sequenceID)]} {
        ...
        if {$class == "" && $seqs($sequenceID,taxonomyNode) != ""} {
            set class [::SeqData::GrowthTemperature::getTemperatureClass $seqs($sequenceID,taxonomyNode)]
        }

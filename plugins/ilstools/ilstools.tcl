...
proc ILStools::create_gui {} {
   ...
   labelframe $f.doc 
   label $f.doc.head -text "Implicit Ligand Sampling (ILS)" -font {Helvetica 10 bold} 
   ...
   menubutton $f.head.probemol -relief raised -bd 2 -direction flush \
       -textvariable ::ILStools::probemol \
   ...
   # Resize grid
   label $f.minmax.grid.dimlabel -text "Resize grid:" \
      -justify left -anchor w
...
proc ILStools::update_sel_align {args} {
   ...
   if {$alignmol!=$sysmol} {
      if {[catch {atomselect $alignmol $alignsel} asel]} {
         tk_messageBox -type ok -icon error -title "Selection Error" \
...
# Check if the given molecule $mol is valid.
...
proc ILStools::shift_to_center {sel {refid none}} {
   ...
   vmdcon "Shifting selection to center for all frames"
...
proc ILStools::wrapnow {} {
   ...
   vmdcon "Wrapping solvent into periodic boundaries:"
   package require pbctools
...
proc ILStools::check_input {} {
   ...
   if {![llength $mapfilename]} {
      tk_messageBox -type ok -icon error -title "Missing DX File Name" \
         -message "Please specify name of dx file for the reulting map!"
      return 0
   }
   if {![llength $prmfiles]} {
...
proc ILStools::write_input {} {
   ...
   puts $file ""
   puts $file "# If you have a CUDA enables Nvidia GPU VMD will use the GPU for"
   ...
   puts $file "    -T \$T -first \$first -last \$last \\"
   puts $file "    -o \$dxfile"
   puts $file ""

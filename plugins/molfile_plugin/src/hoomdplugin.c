/***************************************************************************
 *
 * HOOMD style XML format data/topology file reader and writer
 *
 ...
 *      $Revision: 1.12 $       $Date: 2009/07/25 01:47:18 $
 *
 ***************************************************************************/

/*
 *  HOOMD xml topology/trajectory file format:
 *
 * http://www.ameslab.gov/hoomd/doc/page_xml_file_format.html
 *
 * NOTES: 
 * - all XML elements are converted to lowercase before parsing, so case 
 *   will be ignored. on writing only lowercase will be written.
 ...
 *   or version 1.0 the those nodes will be ignored.
 *
 ***************************************************
...
<hoomd_xml version="1.1">
  <!-- comments -->
  <configuration time_step="0" N="3" ntypes="2">
    <box Units ="sigma"  Lx="49.914" Ly= "49.914" Lz="49.914" />
    ...
    <diameter units="sigma" num="3">
      1.0 1.0 2.0
    </diameter>
    <mass units="mass" num="3">
      1.0 1.0 2.0
    </mass>
    <wall>
 ...
 * do as well, but the one used in hoomd is in c++. :-( */
#include <expat.h>
...
#define HOOMD_MASS      8       /**< atom mass tag        */
#define HOOMD_DIAMETER  9       /**< atom diameter tag    */
#define HOOMD_BOND     10       /**< bond section tag     */
#define HOOMD_ANGLE    11       /**< angle section tag    */
#define HOOMD_DIHEDRAL 12       /**< dihedral section tag */
#define HOOMD_IMPROPER 13       /**< improper section tag */
#define HOOMD_WALL     14       /**< wall description     */
#define HOOMD_COORD    15       /**< coordinate tag       */
#define HOOMD_UNKNOWN  16       /**< unknown tag          */
#define HOOMD_NUMTAGS  17       /**< number of known tags */

static const char *hoomd_tag_name[HOOMD_NUMTAGS] = {
  "(none)", "hoomd_xml", "configuration", "box", "position", "image", 
  "velocity", "type", "mass", "diameter", "bond", "angle", "dihedral",
  "improper", "wall", "coord", "(error)"
};
                        ...
                        const XML_Char **attr)
{
  ...
  if (strcmp(elmt,"hoomd_xml") == 0) {
    d->currtag[d->parsedepth] = HOOMD_XML;
    for (i=0; attr[i]; i+=2) {
    ...
    if (d->majv < 0) {
      vmdcon_printf(VMDCON_INFO, "hoomdplugin) No version attribute found "
                    "on <hoomd_xml> element assuming version=\"1.0\"\n");
      d->majv = 1;
      d->minv = 0;
    }
    
    /* new frame */
  } else if (strcmp(elmt,"configuration") == 0) {
    d->currtag[d->parsedepth] = HOOMD_CONFIG;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if(strcmp(key,"time_step") == 0) {
#if vmdplugin_ABIVERSION > 10
        d->ts.physical_time = atof(value) * d->mytau;
#else 
        ;
#endif
    ...
    /* new box dimensions */
  } else if (strcmp(elmt,"box") == 0) {
    d->currtag[d->parsedepth] = HOOMD_BOX;
    d->ts.A     = d->ts.B    = d->ts.C     = 0.0f;
    d->ts.alpha = d->ts.beta = d->ts.gamma = 90.0f;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key,"units") == 0) {
        if (strcmp(value,"sigma") == 0) {
          d->boxscale = d->mysigma;
        } else { /* unknown unit */
          vmdcon_printf(VMDCON_WARN,
                        "hoomdplugin) unknown box length unit: %s. "
                        "no conversion to angstrom done.\n", value);
          d->boxscale = 1.0f;
        }
    ...
    /* coordinates */
  } else if (strcmp(elmt,"position") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_POS;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key,"units") == 0) {
        if (strcmp(value,"sigma") == 0) {
          d->posscale = d->mysigma;
        } else { /* unknown unit */
          vmdcon_printf(VMDCON_WARN,
                        "hoomdplugin) unknown position length unit: %s. "
                        "no conversion to angstrom done.\n", value);
          d->posscale = 1.0f;
        }
    ...
    /* boxcrossings / image counters  */
  } else if (strcmp(elmt,"image") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_IMAGE;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key,"num") == 0) {
        ;                  /* XXX: number of images. use for check. */
      } else {
        XML_UNK_ATTR(element, key, value);
      }
    }
    ...
    /* velocities */
  } else if (strcmp(elmt,"velocity") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_VEL;
#if vmdplugin_ABIVERSION > 10
    d->ts_meta.has_velocities = 1;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key, "units") == 0) {
        if (strcmp(value,"sigma/tau") == 0) {
          d->velscale = d->mysigma/d->mytau;
        } else { /* handle unknown units better */
          vmdcon_printf(VMDCON_WARN,
                        "hoomdplugin) unknown velocity unit: %s. "
                        "no conversion to angstrom/fs done.\n", value);
          d->velscale = 1.0f;
        }
      } else if (strcmp(key,"num") == 0) {
  ...
  } else if (strcmp(elmt,"mass") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_MASS;
    ...
    for (i=0; attr[i]; i+=2) {
      ...
      if (strcmp(key,"units") == 0) {
        if (strcmp(value,"mass") == 0) {
          ...
          vmdcon_printf(VMDCON_WARN,
                        "hoomdplugin) unknown mass unit: %s. "
                        "no conversion to AMU is done.\n", value);
          d->massscale = 1.0f;
        }
      } else if (strcmp(key,"num") == 0) {
  ...
  } else if (strcmp(elmt,"diameter") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_DIAMETER;
    d->optflags |= MOLFILE_RADIUS;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key,"units") == 0) {
        if (strcmp(value,"sigma") == 0) {
          d->radscale = d->mysigma;
        } else { /* unknown unit */
          vmdcon_printf(VMDCON_WARN,
                        "hoomdplugin) unknown diameter length unit: %s. "
                        "no conversion to angstrom done.\n", value);
          d->radscale = 1.0f;
        }
      } else if (strcmp(key,"num") == 0) {
  ...
  } else if (strcmp(elmt,"type") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_TYPE;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key,"num") == 0) {
  ...
  } else if (strcmp(elmt,"bond") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_BOND;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key,"num") == 0) {
        ;                 /* XXX: number of bonds. use for check. */
      } else {
        XML_UNK_ATTR(element, key, value);
      }
    }

  } else if (strcmp(elmt,"angle") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_ANGLE;
    if (d->majv == 1 && d->minv < 1) {
      if (d->numangles < 0) {
        vmdcon_printf(VMDCON_WARN, "hoomdplugin) Found <angle> section in "
  ...
  } else if (strcmp(elmt,"dihedral") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_DIHEDRAL;
    if (d->majv == 1 && d->minv < 1) {
      if (d->numdihedrals < 0) {
        vmdcon_printf(VMDCON_WARN, "hoomdplugin) Found <dihedral> section in "
                      "a pre-1.1 version format file. Ignoring it...\n");
      }
    }
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key,"num") == 0) {
        ;                 /* XXX: number of dihedrals. use for check. */
      } else {
        XML_UNK_ATTR(element, key, value);
      }
    }

  } else if (strcmp(elmt,"improper") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_IMPROPER;
    if (d->majv == 1 && d->minv < 1) {
      if (d->numimpropers < 0) {
        vmdcon_printf(VMDCON_WARN, "hoomdplugin) Found <improper> section in "
                      "a pre-1.1 version format file. Ignoring it...\n");
      }
    }
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key,"num") == 0) {
        ;                 /* XXX: number of impropers. use for check. */
      } else {
        XML_UNK_ATTR(element, key, value);
      }
    }

  } else if (strcmp(elmt,"wall") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_WALL;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      XML_UNK_ATTR(element, key, value);
    }

  } else if (strcmp(elmt,"coord") == 0) {
    d->counter = 0;
    d->currtag[d->parsedepth] = HOOMD_COORD;
    for (i=0; attr[i]; i+=2) {
      XML_ATTR_COPY(attr, i, key, value);
      if (strcmp(key,"units") == 0) {
        if (strcmp(value,"sigma") == 0) {
          ...
          vmdcon_printf(VMDCON_WARN,
                        "hoomdplugin) unknown wall length unit: %s. "
                        "no conversion to angstrom done.\n", value);
          ; /* do nothing. we don't do anything with walls, yet.*/
        }
    ...
    vmdcon_printf(VMDCON_WARN,
                  "hoomdplugin) Ignoring unknown XML element: %s.\n", element);
  }
}
 ...
 *  and so on and store it for later use. */
  static void *open_hoomd_read(const char *filename, const char *filetype, 
    ...
    if (data) {
      ...
      /* allow overriding hardcoded sigma/tau values from environment */
      envvar = getenv("VMDHOOMDSIGMA");
      if (envvar) data->mysigma = atof(envvar);
      envvar = getenv("VMDHOOMDTAU");
      if (envvar) data->mytau = atof(envvar);
      envvar = getenv("VMDHOOMDMASS");
      if (envvar) data->massscale = atof(envvar);
      ...
      do {
        if (MOLFILE_ERROR == hoomd_parse_line(data)) {
          vmdcon_printf(VMDCON_ERROR, "hoomdplugin) XML Parse error "
                      "while reading HOOMD data file '%s'\n", filename);
      ...
      if ( data->majv > HOOMD_FORMAT_MAJV ) {
        vmdcon_printf(VMDCON_ERROR, "hoomdplugin) Encountered incompatible "
                      "HOOMD data file format version '%d.%d.\n", 
                      data->majv, data->minv);
        vmdcon_printf(VMDCON_ERROR, "hoomdplugin) This plugin supports only "
                      "HOOMD data files up to version '%d.%d'.\n",
        ...
        if ( (data->majv == HOOMD_FORMAT_MAJV) && 
          ...
          vmdcon_printf(VMDCON_WARN, "hoomdplugin) Encountered newer HOOMD "
                        "data file format version '%d.%d'.\n",
                        data->majv, data->minv);
          vmdcon_printf(VMDCON_WARN, "hoomdplugin) This plugin supports HOOMD "
      ...
      if (data->numatoms < 0) {
        vmdcon_printf(VMDCON_ERROR, "hoomdplugin) Could not determine "
                      "number of atoms in HOOMD data file '%s'\n", filename);
        XML_ParserFree(data->p);
        data->p = NULL;
        SAFE_FREE(data->filename);
        fclose(data->fp);
...
static int read_hoomd_structure(void *mydata, int *optflags, 
  ...
  for (i=0, a=atoms; i < data->numatoms; ++i, ++a) {
    ...
    if (data->optflags & MOLFILE_RADIUS) {
      a->radius *= data->radscale;
    } else {
      a->radius = get_pte_vdw_radius(a->atomicnumber);
    }

    if (data->optflags & MOLFILE_MASS) {
      a->mass *= data->massscale;
    } else {
...
static int read_hoomd_timestep(void *mydata, int natoms, 
  ...
  if (ts != NULL) { 
    ...
    for (i=0; i<natoms; ++i) {
      ts->coords[3*i+0] = data->ts.coords[3*i+0] * data->posscale 
        + data->ts.A * data->imagecounts[3*i+0];
      ts->coords[3*i+1] = data->ts.coords[3*i+1] * data->posscale 
        + data->ts.B * data->imagecounts[3*i+1];
      ts->coords[3*i+2] = data->ts.coords[3*i+2] * data->posscale 
    ...
    if (ts->velocities != NULL) {
      for (i=0; i<natoms; ++i) {
        ts->velocities[3*i+0] = data->ts.velocities[3*i+0] * data->velscale;
        ts->velocities[3*i+1] = data->ts.velocities[3*i+1] * data->velscale;
        ts->velocities[3*i+2] = data->ts.velocities[3*i+2] * data->velscale;
      }
    }
#endif
    ts->A = data->ts.A;
...
static void *open_hoomd_write(const char *filename, const char *filetype, 
  ...
  fd = fopen(filename, "w");
  if (!fd) { 
    vmdcon_printf(VMDCON_ERROR, "hoomdplugin) Unable to open HOOMD data file %s "
                  "for writing\n", filename);
    return NULL;
  }
...
static int write_hoomd_structure(void *mydata, int optflags, 
  ...
  if (optflags & MOLFILE_RADIUS) {
    fprintf(data->fp, "<diameter units=\"sigma\" num=\"%d\">\n", data->numatoms);
    for (i=0; i < data->numatoms; ++i) {
      fprintf(data->fp,"%f\n", 2.0f*atoms[i].radius/data->mysigma);
    }
    fputs("</diameter>\n", data->fp);
  }
  if (optflags & MOLFILE_MASS) {
    fprintf(data->fp, "<mass units=\"mass\" num=\"%d\">\n", data->numatoms);
    for (i=0; i < data->numatoms; ++i) {
      fprintf(data->fp,"%f\n", atoms[i].mass);
    }
    fputs("</mass>\n", data->fp);
  ...
  if ( (data->numdihedrals > 0) && (data->dihedral != NULL) ) {
    fprintf(data->fp, "<dihedral num=\"%d\">\n", data->numdihedrals);
  ...
  if ( (data->numimpropers > 0) && (data->improper != NULL) ) {
    fprintf(data->fp, "<improper num=\"%d\">\n", data->numimpropers);
...
static int write_hoomd_timestep(void *mydata, const molfile_timestep_t *ts) {
  hoomd_data_t *data = (hoomd_data_t *)mydata; 
  const float *pos;
  float scalef, px, py, pz;
  int i, *img;

  if (data->numframe != 0) {
#if vmdplugin_ABIVERSION > 10
    fprintf(data->fp,"<configuration time_step=\"%d\" natoms=\"%d\">\n", 
            (int)(ts->physical_time/data->mytau+0.5f), data->numatoms);
#else
    fprintf(data->fp,"<configuration time_step=\"%d\" natoms=\"%d\">\n", 
            data->numframe, data->numatoms);
#endif
  ...
  scalef = 1.0f/data->mysigma;
  fprintf(data->fp, "<box units=\"sigma\" lx=\"%f\" ly=\"%f\" lz=\"%f\" />\n",
          scalef*ts->A, scalef*ts->B, scalef*ts->C);
...
#define HOOMD_CONV_POS(crd, box, p, i)  \
  ...
  fprintf(data->fp, "<position units=\"sigma\" num=\"%d\">\n", data->numatoms);
  pos = ts->coords;
  ...
  if (ts->velocities != NULL) {
    const float *vel;
    scalef = data->mytau / data->mysigma;
    
    fprintf(data->fp, "<velocity units=\"sigma/tau\" num=\"%d\">\n", data->numatoms);
    vel = ts->velocities;
    for (i = 0; i < data->numatoms; ++i) {
      fprintf(data->fp, "%.6f %.6f %.6f\n", scalef*vel[0], scalef*vel[1], scalef*vel[2]);
      vel += 3;
    }
    fputs("</velocity>\n",data->fp);
  }
...
static void close_hoomd_write(void *mydata) {
  ...
  /* we have not written positions, so the configuration tag is still open. */
  if (data->numframe == 0) {
    fputs("</configuration>\n", data->fp);
  }
  fputs("</hoomd_xml>\n", data->fp);
...
VMDPLUGIN_API int VMDPLUGIN_init() {
  ...
  plugin.name = "hoomd";
  plugin.prettyname = "HOOMD XML File";

 ...
 *      $Revision: 1.182 $       $Date: 2009/08/01 04:21:42 $
 *
 ***************************************************************************/
...
/* Read MCSCF data */
static int get_mcscf(gamessdata *data);
...
static int check_add_wavefunctions(gamessdata *data,
                                   qm_timestep_t *ts);
...
static int get_singlepoint_gradient(gamessdata *, qm_timestep_t *ts);
...
static int get_int_hessian(gamessdata *);
...
static void *open_gamess_read(const char *filename, 
  ...
  if (have_gamess(data)==TRUE) {
    if (data->have_pcgamess) {
      printf("gamessplugin) PC GAMESS is not yet supported.\n");
      return NULL;
    }
...
static int have_gamess(gamessdata *data) 
{
  ...
  program = goto_keyline2(data->file,
                          "PC GAMESS version",
                          "GAMESS VERSION =", NULL);
  if (program==1) {
...
static int get_input_structure(gamessdata *data) {
  ...
  if (goto_keyline2(data->file,
    ...
    if (pass_keyline(data->file,
      ...
      if (!get_fmoxyz(data->file, &data->initatoms, bohr, &numatoms)) {
...
static int get_final_properties(gamessdata *data) {
  ...
  if (data->runtype == GRADIENT ||
      data->runtype == HESSIAN) {
    if (get_singlepoint_gradient(data, ts)) {
...
static int fill_basis_arrays(gamessdata *data) {
  ...
  for (i=0; i<data->num_basis_atoms; i++) {
    ...
    for(j=0; j<data->numatoms; j++) {
      ...
      if (!strcmp(data->initatoms[j].type, basisname)) {
        found = 1;
        break;
      }
    }
...
static int get_traj_frame(gamessdata *data, qm_atom_t *atoms,
  ...
  if (data->runtype==OPTIMIZE || data->runtype==SADPOINT) {
    ...
    if (!get_coordinates(data->file, &data->initatoms, units, &natoms)) {
    ...
    if (pass_keyline(data->file, "HAS ENERGY VALUE",
      ...
      for (i=0; i<natoms; i++) {
        ...
        if (n!=4 || strcmp(atname, data->initatoms[i].type)) break;
        data->initatoms[i].x = x;
        data->initatoms[i].y = y;
        data->initatoms[i].z = z;
      }
      if (i!=natoms) {
  ...
  /* XXX could merge this with OPTIMIZE/SADPOINT */
    ...
    units = !strcmp(word, "(BOHR)");
    eatline(data->file, 2);
    if (!get_coordinates(data->file, &data->initatoms, units, &numuniqueatoms)) {
      ...
      symmetry_expand(&data->initatoms, numuniqueatoms, natoms,
...
static int analyze_traj(gamessdata *data) {
  ...
  while (1) {
    ...
    if (strstr(line, nserch) ||
        ...
        (data->runtype==MEX && strstr(line, "NSERCH=")==line)) {
      ...
      if (data->runtype==SURFACE) {
        int ret = goto_keyline2(data->file,
...
static int get_scfdata(gamessdata *data, qm_timestep_t *ts) {
  ...
  if (!goto_keyline2(data->file, "ITER EX", "ITER     TOTAL",
                     "ITER    TOTAL", NULL)) {
    fseek(data->file, filepos, SEEK_SET);
    ts->num_scfiter = 0;
    return FALSE;
 ...
 *
 **********************************************************/
static int get_wavefunction(gamessdata *data, qm_timestep_t *ts,
...
static int get_gradient(gamessdata *data, qm_timestep_t *ts) {
  ...
  if (goto_keyline2(data->file, "GRADIENT (HARTREE",
...
static int get_singlepoint_gradient(gamessdata *data, qm_timestep_t *ts) {
  int numgrad=0;
  int numread;
  char buffer[BUFSIZ];
  long filepos;
 ...
 *
 **********************************************************/
static int get_int_hessian(gamessdata *data) {
  char buffer[BUFSIZ];
  double hess[5];
  int i = 0, j = 0, k = 0, l = 0;
 ...
 * The last argument must be NULL signifying the end of
 * the keystring argument list.
 ...
 * If the keystring wasn't found before EOF then
 * the file is rewound to the position where the search
 * started and 0 is returned.
 * If stopstring is NULL then the search stops
 * at EOF. */
static int goto_keyline2(FILE *file, ...) {
  char buffer[BUFSIZ];
...
#if 1
/* places file pointer AT THE BEGINNING of the line containing 
   the keystring */
static int goto_keyline(FILE *file, const char *keystring,
                        const char *keystring2) {
  char buffer[BUFSIZ];
...
static int have_keyline(FILE *file, const char *keystring,
  ...
  found = pass_keyline(file, keystring, keystring2);
 ...
 * Provide the array and number of unique atoms, the total 
 * number of atoms, the pointgroup string and the order of
 * the highest axis. The atoms array will be resized
